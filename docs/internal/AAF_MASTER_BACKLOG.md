...
| Status | **SOLVED**（AAF-v0.4-TASK-011 交付，2026-08-29；implementation + 全量测试通过；独立 WorkBuddy 验证 + Codex 审查为 route 最后闸门） |
| Priority | P1 |
...
| Current Implementation | （2026-08-29 完成态）`ai_agent_framework/workbuddy_retry.py`：① failure classification 只基于真实 CLI evidence——RETRYABLE_TRANSIENT = TimeoutExpired / exit=0+空输出 / stderr 含 gateway placeholder-only 证据；NON_RETRYABLE = missing executable / 非零退出无 transient evidence（auth/config/CLI fatal）/ spawn 失败；② WorkBuddy-only bounded retry：initial + 1 retry（`AAF_WORKBUDDY_MAX_ATTEMPTS` 可配），每次 attempt 复用同一 invocation（same agent / same current model & default config / same prompt / same workspace / same role，绝不换模型/provider/付费层级）；③ per-attempt timeout 默认 900s（实测成功 stage ~556s，留余量；`AAF_WORKBUDDY_TIMEOUT` 可配）替代统一 3600s；④ 整体 stage budget 硬上限（默认 = attempts×timeout+backoff；`AAF_WORKBUDDY_STAGE_BUDGET` 可配），per-attempt timeout 不超过剩余 budget；⑤ bounded backoff 默认 30s（`AAF_WORKBUDDY_BACKOFF` 可配）；⑥ timeout → Windows `taskkill /PID /T /F` 树杀 + 有界等待 + 管道排空，spawn 前断言无并发 child（无 orphan/zombie/pipe leak）；⑦ 有效输出（含业务 FAIL verdict）立即停止 retry，verdict 归 Framework authority（structured result / blocking provenance / fail-closed 语义不变）；⑧ retries 用尽 → `WorkBuddyRetriesExhausted`（fail closed → FRAMEWORK_ERROR 带完整 attempt 历史 → Codex 不运行 → WAITING）；⑨ attempt telemetry（`workbuddy_attempts.json` machine artifact：attempt_count / 每 attempt 状态/类别/原因/超时/退出码/耗时/stderr tail / stage 总墙钟）+ stage result `execution_retries` 摘要 + REPORT Model Observation 行 `attempts=N`（仅 retried 时）；⑩ 测试全 mocked/fake CLI（零真实 gateway/积分消耗），矩阵 A–K + CLOSURE-002/003 双 incident regression + 真实 python child 超时清理（Windows tasklist 验证无 orphan）+ runner 集成（Codex gating 正负）共 42 项新测试；全量 non-GUI 1265 passed |
...
| Current Implementation | 无（零 routing 代码） |
