...
A2 Delivered（2026-08-31，AAF-v0.5-A2-SHADOW-ROUTING-001 — Shadow Routing 首片，observation-only）:
  ...
  - 实际执行隔离：runner / router / adapters / cost_guard / report / lifecycle 零修改、
    ...
    未实现 A3-A6 / 未做健康轮询
A2 Status: STARTED（2026-08-31，AAF-v0.5-A2-SHADOW-ROUTING-001：首片 deterministic
  ...
  candidate）。A2 未 COMPLETE——实况 qualification 观测/后续 A2 slice 留待后续）
Next mainline: v0.5 -> A2 Shadow Routing（IN PROGRESS —— 首片已交付，见上方 A2 Delivered；
  A3 Hermes Free Auto Routing 未启动；Hermes free-model 可用性/稳定性为后续路由（A2+ 影子/
  实况观测）的 runtime qualification concern，不假定 FREE = healthy；详见
  AAF-v0.5-A2-SHADOW-ROUTING-001 REPORT 与 AAF-v0.5-A1-CLOSURE-AUDIT-001-FIX-001 REPORT）
