...
- **当前任务区**：Task ID、Task Name、当前阶段、当前 Agent（Hermes / WorkBuddy / Codex）、
  ...
  停止状态（正在运行 / 请求停止 / 正在取消 / 已取消 / 已完成 / 无法安全停止）、
...
- **疑似停滞提示**：任务 RUNNING 且最近 10 分钟无活动时显示黄色提示
  ...
  终止或修改任务状态
- **操作**：查看日志、查看任务目录（打开任务输出目录）、**停止当前任务**（仅
  执行中可用）、**强制停止**（仅任务长时间未退出且所有权确认时出现）、关闭、
  重启 Bridge、退出 AAF

要点：

- 状态窗口本身**不直接修改任务结果文件**（task.json / run.json 由 Framework 自己写）；
  「停止当前任务」只发送 `cancel.request` 软取消请求（任务在当前阶段自然结束后
  以「已取消」收敛）；「强制停止」必须经过二次中文确认，只终止已通过所有权验证
  的任务进程树，最终状态仍由任务框架收敛（见下方「取消任务」）。
...
- **Process Ownership Protocol（TASK-005-B）**：每次真实 launch 生成唯一 `launch_id`，
  ...
  - **wait thread canonical-aware**：进程退出后跟随 canonical terminal；派生产物
    ...
    推导。
- **尚未交付 / 边界（TASK-005-C 已交付后的状态）**：
  - 状态窗口「停止当前任务」+ 二次确认的「强制停止」按钮已交付（005-C），
    具体交互见下方「状态窗口停止 / 强制停止任务（TASK-005-C）」小节。
  - **不提供**：Tray 菜单停止项（§12.2 Tray 停止项留待后续阶段，不在 005-C 范围）、
    Phase F（项目切换 / Duplicate UX）。

> 状态窗口的停止 / 强制停止是**控制请求**：最终任务状态永远由 Framework（Core）裁决——
> 正常完成先于取消请求到达时，任务保持原终态（窗口显示「任务已先完成」），
> 取消请求被吸收（§6A.2 / §6A.13）。

### 状态窗口停止 / 强制停止任务（TASK-005-C，Phase E 最后交付）

v0.4 Phase E（TASK-005-C）已在状态窗口交付安全的 Stop / Cancel UX：

- **停止状态行**：窗口显示当前任务的停止状态——
  「正在运行」（可停止）→「请求停止」（停止请求已发送，正在等待任务安全退出）→
  「正在取消」（软取消超时任务仍未退出，出现「强制停止」选项）→「已取消」/
  「已完成」/「无法安全停止」。
- **停止当前任务（soft cancel 优先）**：点击 → 中文停止确认窗（[确认停止] [取消]）
  → 只写 `cancel.request`（软取消契约，走 Core-owned `state.lock` 序列化写）。
  任务在当前阶段自然结束后收敛为 CANCELLED；已完成阶段的 Agent 结果全部保留；
  若任务在取消前已完成（race），窗口显示「任务已先完成」，原终态保持不变。
- **软取消超时（默认 30s，`force_cancel_soft_timeout` 可配）**：任务仍未退出时，
  窗口明确提示「任务仍未退出，可选择强制停止」——**不会自动强制终止**。
- **强制停止（必须二次确认）**：只有后端明确判定 force eligible（软取消超时 +
  11 项 ownership 三方验证通过）时才显示「强制停止」按钮；点击后出现红色风险
  确认窗（[确认强制停止] [取消]），第二次确认后才执行 verified 进程树终止 +
  结构化 force evidence + Core recovery finalizer 收敛 CANCELLED。所有权
  UNCERTAIN / STALE / 无法确认时窗口显示「无法安全停止」，不提供强制停止。
- **重启恢复**：Bridge / 状态窗口重启或重新打开后，停止状态从
  `task.json` / `cancel.request` / launch registry 等持久 artifacts 恢复，
  不依赖窗口内存。
