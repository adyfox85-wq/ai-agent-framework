...
Next mainline: v0.5 -> A2 Shadow Routing = CLOSED / COMPLETE / SYNCED（2026-08-31
  ...
  observation/calibration/runtime requalification（health/quarantine 属之）——均
  NOT IMPLEMENTED、未进入；MEDIUM/HIGH 自动模型选择属 A4-A6（broader model
  routing，非 A3 remaining scope）；
...
A3 Status: CLOSED / COMPLETE / SYNCED（2026-09-01 AAF-v0.5-A3-CLOSE-001 依已完成的
  ...
  双 ref）；A4-A6 未进入——正式边界：A4 = WorkBuddy/economic/multi-agent routing、
  A5 = fallback / Cost Gate UX、A6 = observation/calibration/runtime
  requalification（health/quarantine 属之），均 NOT IMPLEMENTED；MEDIUM/HIGH 自动
  模型选择属 A4-A6（broader model routing）；本关闭 docs-only（closure record =
...
A4 Active Economic Routing Delivered（2026-09-02，AAF-v0.5-A4-WORKBUDDY-ECONOMIC-ROUTING-001 — **A4 = STARTED**（第一条 active-routing slice））:
  ...
  - **A4 = STARTED**（第一条 slice = LOW-risk WorkBuddy economic active routing）；
    broader MEDIUM/HIGH / multi-agent 路由仍为未来 A4 工作（NOT IMPLEMENTED）；
    A5（fallback / Cost Gate UX）/ A6（observation/calibration/runtime
    requalification）未进入；A0-A3 = CLOSED / COMPLETE / SYNCED 不变；report =
...
A4 MEDIUM Active Economic Routing Delivered（2026-09-02，AAF-v0.5-A4-WORKBUDDY-ECONOMIC-ROUTING-002 — **A4 保持 STARTED**，active-routing risk 域 = explicit LOW + MEDIUM）:
  ...
  - fresh-runner N+1 **6/6**（N1 MEDIUM 真实 Auto / N1b MEDIUM 受控 routed /
    ...
    Requirement 17/18）
  - **A4 保持 STARTED**（LOW + MEDIUM WorkBuddy economic routing implemented；
    HIGH / broader multi-agent 仍未来 A4 工作，Requirement 19/20），A5/A6 未进入；
