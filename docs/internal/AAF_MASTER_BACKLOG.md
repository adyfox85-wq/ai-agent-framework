...
| Related | RW-007（Agent executable discovery reliability——MISSING_COMMAND 仍永久失败不重试）；CAP-003（Future Model Routing——A3 LOW-risk Hermes FREE active routing = IMPLEMENTED + CLOSED；A4 LOW + MEDIUM WorkBuddy economic active routing = IMPLEMENTED + CLOSED / COMPLETE（A4 = CLOSED——2026-09-02 AAF-v0.5-A4-CLOSE-001 正式关闭）；HIGH / CRITICAL WorkBuddy 与 multi-agent / Codex routing = A4+ future scope NOT IMPLEMENTED；其余 broader model routing 属 A4-A6） |
...
| Status | **IMPLEMENTED（A3 slice：LOW-risk Hermes FREE active routing；A4 slice：LOW + MEDIUM WorkBuddy economic active routing——A4 = CLOSED / COMPLETE（2026-09-02 AAF-v0.5-A4-CLOSE-001））/ NOT IMPLEMENTED（A4+ future scope：HIGH / CRITICAL WorkBuddy routing、multi-agent / Codex routing；其余 broader model routing 属 A4-A6）** |
| Priority | P2 |
...
| Remaining Gap | A4 正式 scope = **无 remaining gap（LOW + MEDIUM WorkBuddy economic active routing = IMPLEMENTED + CLOSED / COMPLETE——2026-09-02 AAF-v0.5-A4-CLOSE-001 正式关闭，closure 已执行）**；A4+ future scope NOT IMPLEMENTED：HIGH / CRITICAL WorkBuddy routing（HIGH 前置 = T2-or-better WorkBuddy 候选 prerequisite qualification evidence）、multi-agent / Codex routing（前置 = 独立 scope/design 定义）；其余 broader model routing（MEDIUM/HIGH Hermes 自动模型选择与 cheapest-model selection）仍属 A4-A6 未实现；A5 = fallback escalation / Cost Gate UX 与 free→paid switching / user confirmation dialog；A6 = observation-calibration-runtime requalification 与 health/quarantine |
...
| Do Not Forget | A3 LOW slice（LOW-risk Hermes FREE active routing）已实现并关闭；A4 LOW + MEDIUM WorkBuddy economic active routing = IMPLEMENTED + CLOSED / COMPLETE（A4 = CLOSED——2026-09-02 AAF-v0.5-A4-CLOSE-001 正式关闭）；但 **broader Model Routing 未实现**（HIGH / CRITICAL / multi-agent = A4+ future scope；其余 MEDIUM/HIGH 等属 A4-A6）；Free→Paid 切换等决策层必须用户批准（A5 Cost Gate UX） |
...
| Do Not Forget | 本 A0 只做 Hermes 保护层；不得据此宣称完整 Model Routing 已实现（CAP-003 完整 Model Routing 仍 NOT IMPLEMENTED，属 A4-A6 与 A4+ future scope；已实现并关闭的仅 A3 LOW-risk Hermes FREE slice + A4 LOW + MEDIUM WorkBuddy economic slice（A4 = CLOSED / COMPLETE——2026-09-02 AAF-v0.5-A4-CLOSE-001 正式关闭））；授权 env 是 per-run 一次性，不是永久/全局 bypass |
...
| CAP-003 | Future Model Routing（政策登记；A3 LOW-risk Hermes FREE active routing = IMPLEMENTED + CLOSED；A4 LOW + MEDIUM WorkBuddy economic active routing = IMPLEMENTED + CLOSED / COMPLETE（A4 = CLOSED——2026-09-02 AAF-v0.5-A4-CLOSE-001）；HIGH/CRITICAL/multi-agent = A4+ future scope NOT IMPLEMENTED；其余 broader model routing 属 A4-A6） | IMPLEMENTED（A3 LOW slice + A4 LOW/MEDIUM WorkBuddy slice，均已关闭；HIGH/CRITICAL/multi-agent = A4+ 与 MEDIUM-HIGH Hermes/fallback/health 等属 A4-A6 未实现） | P2 |
