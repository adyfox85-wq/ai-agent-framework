...
Next mainline: v0.5 -> A2 Shadow Routing = CLOSED / COMPLETE / SYNCED（2026-08-31
  ...
  Hermes Free Auto Routing（actual routing，CAP-003）——首片 = AAF-v0.5-A3-HERMES-
  FREE-ROUTING-001 已交付（LOW-risk LOCAL_FREE active routing 真实生效，见上方
  A3 Delivered；A3 = STARTED，未标 COMPLETE——WorkBuddy 独立验证 + Codex APPROVE +
  Planner 正式判定前置）；A3 后续范围（MEDIUM/HIGH 自动选择 / fallback / health /
  quarantine / promotion / A4-A6）均未实现；
  Hermes free-model 可用性/稳定性为 A2+（RW-030 实况观测）的 runtime qualification
  concern，不假定 FREE = healthy；
  详见 AAF-v0.5-A2-CLOSE-001-REPORT.md 与 AAF-v0.5-A1-CLOSURE-AUDIT-001-FIX-001 REPORT）
...
A3 Delivered（2026-09-01，AAF-v0.5-A3-HERMES-FREE-ROUTING-001 — LOW-risk Hermes
  ...
  - 边界（Requirement 9）：未实现 WorkBuddy/Codex routing、MEDIUM/HIGH 自动模型选择、
    ...
    promotion、A4-A6；A3 = STARTED（**未标 COMPLETE**；WorkBuddy 独立验证 + Codex
    审查 + Planner 判定为后续闸门）
A3 Status: STARTED（2026-09-01 AAF-v0.5-A3-HERMES-FREE-ROUTING-001 交付首片；
  未标 COMPLETE——WorkBuddy 独立验证 + Codex APPROVE + Planner 正式判定前置）
