...
| Do Not Forget | A3 LOW slice（LOW-risk Hermes FREE active routing）已实现并关闭，但 **broader Model Routing 未实现**（MEDIUM/HIGH / fallback / multi-agent 属 A4-A6）；Free→Paid 切换等决策层必须用户批准（A5 Cost Gate UX） |
...
| Do Not Forget | 本 A0 只做 Hermes 保护层；不得据此宣称完整 Model Routing 已实现（CAP-003 完整 Model Routing 仍 NOT IMPLEMENTED，属 A4-A6；仅 A3 LOW-risk FREE slice 已实现并关闭）；授权 env 是 per-run 一次性，不是永久/全局 bypass |
...
| CAP-003 | Future Model Routing（政策登记；LOW-risk Hermes FREE active routing = IMPLEMENTED + CLOSED；broader model routing 仍 NOT IMPLEMENTED，属 A4-A6） | IMPLEMENTED（A3 LOW slice；MEDIUM/HIGH/fallback/health/multi-agent 等属 A4-A6 未实现） | P2 |
