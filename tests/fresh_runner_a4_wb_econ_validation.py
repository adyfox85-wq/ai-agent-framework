...
（精确证明实际 invocation 形状——A4 FIX-001 起，真实 economic facts 下经济
过滤后只有 hy4-preview 一个可信候选 → LOW 任务保持 CodeBuddy Auto；事实层
本身不会自动加 --model）。
...
def main() -> int:
    ...
    scenario_record: dict = {
        ...
        "purpose": (
            ...
            "Since A4 FIX-001 (two-candidate economic routing gate), a LOW task's "
            "WorkBuddy stage stays CodeBuddy Auto under REAL economic facts (only "
            "hy4-preview is economically trustworthy -> gate requires >= 2): N1 "
            "argv is exactly '-p --output-format text -y' (no --model, no "
            "--effort); plus (N2) a fresh-process check proving the economic "
            "observation artifact (economic_facts.json) is generated/readable as "
            "designed (facts_from_dict parses it, freshness matches "
            "classify_freshness at observed_at) and that economics alone never "
            "alters routing authority (selector still picks only deepseek-v4-flash "
            "for LOW workbuddy; the --model comes only from an A4 routing decision "
            "with >= 2 trustworthy candidates, not from the fact layer alone)."
        ),
        "scenarios": {},
    }
    try:
