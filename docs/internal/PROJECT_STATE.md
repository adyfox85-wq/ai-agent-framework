...
A2 Status: CLOSED / COMPLETE / SYNCED（2026-08-31，AAF-v0.5-A2-CLOSE-001 正式关闭：
  ...
  非「后续 A2 slice」，未实现）；actual routing = A3（CAP-003，NOT IMPLEMENTED，
...
Next mainline: v0.5 -> A2 Shadow Routing = CLOSED / COMPLETE / SYNCED（2026-08-31
  ...
  docs/internal/AAF-v0.5-A2-CLOSE-001-REPORT.md）；后续候选 = A3 Hermes Free Auto
  Routing（actual routing，CAP-003，未启动，NOT IMPLEMENTED）与 A2+ runtime
  qualification observation（RW-030，未实现）；Hermes free-model 可用性/稳定性为
  A2+（RW-030 实况观测）的 runtime qualification concern，不假定 FREE = healthy；
