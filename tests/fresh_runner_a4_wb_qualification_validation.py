"""AAF-v0.5-A4-PREREQ-WORKBUDDY-QUALIFICATION-001 — fresh-runner N+1 validation driver.

本任务修改未来 WorkBuddy routing 会消费的 registry qualification 数据，按 TASK
...
本身不会自动加 --model）。
  ...
  N3（fresh-process eligibility check）-> 全新 python 进程证明：新 qualification
                      数据仅影响候选 eligibility（deepseek-v4-flash LOW 下
                      eligible；MEDIUM/HIGH 仍 NO_SHADOW_CANDIDATE；其余 14 候选
                      ineligible），adapters._workbuddy_invocation 零 --model。

用法：python tests/fresh_runner_a4_wb_qualification_validation.py
退出码 = 失败场景数（0 = 全部通过）。运行时证据写入
.aaf/AAF-v0.5-A4-PREREQ-WORKBUDDY-QUALIFICATION-001/fresh-runner-validation/
...
ELIGIBILITY_CHECK = ROOT / "tests" / "fresh_runner_a4_wb_qual_eligibility_check.py"
DEFAULT_EVIDENCE_ROOT = (
    ROOT / ".aaf" / "AAF-v0.5-A4-PREREQ-WORKBUDDY-QUALIFICATION-001"
...
:version
echo Hermes Agent v0.20.5 fake WB-QUAL-N1
exit /b 0
...
rem AAF-v0.5-A4-PREREQ-WORKBUDDY-QUALIFICATION-001 fresh-runner N+1 fake CodeBuddy
...
rem NO --model / --effort may appear even though the registry now qualifies one
rem WorkBuddy candidate.
...
rem AAF-v0.5-A4-PREREQ-WORKBUDDY-QUALIFICATION-001 fresh-runner N+1 fake Codex CLI
rem (Reviewer stage).
...
def main() -> int:
    ...
    scenario_record: dict = {
        ...
        "task_id": "AAF-v0.5-A4-PREREQ-WORKBUDDY-QUALIFICATION-001",
        "purpose": (
            ...
            "even though the registry now qualifies deepseek-v4-flash as a WorkBuddy "
            "candidate; plus (N3) a fresh-process eligibility check proving the new "
            "qualification data only affects candidate eligibility (LOW selector sees "
            "deepseek-v4-flash; MEDIUM/HIGH still NO_SHADOW_CANDIDATE; 14 others "
            "ineligible) and never alters routing authority / invocation."
        ),
        "scenarios": {},
    }
    try:
    ...
    print(f"fresh-runner A4-WB-QUALIFICATION N+1: failures={failures}")
    return failures
