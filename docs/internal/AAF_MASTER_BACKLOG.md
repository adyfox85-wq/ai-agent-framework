...
| Current Implementation | 无（零 routing 代码）。注（2026-08-31，AAF-v0.5-A2-SHADOW-ROUTING-001）：A2 Shadow Routing 已 STARTED——`ai_agent_framework/shadow_routing.py` deterministic **shadow-selection engine**（假设性 / 非权威 / 零执行影响，live 路径零 import）；本 CAP-003 指**实际** Model Routing（切换 / fallback / 升级），仍 **NOT IMPLEMENTED** |
