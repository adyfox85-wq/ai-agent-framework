"""AAF-v0.5-A0-PAID-GUARD-001-FIX-005 fresh-runner 验证驱动（Run N+1）。

FIX-005（FIX-004 re-issue）：paid admission 无持久化 filesystem authority
（state_dir=None / 不可用）→ fail closed。本次变更涉及 AAF 准入权威，同 run
证据不足：本驱动从**全新 python 进程**（post-change 代码）复现并证明：

  S1  独立 fresh 进程 + matched paid auth + state_dir=None
      -> BLOCKED_COST_APPROVAL（零 ALLOWED；_CONSUMED_AUTHS 绝不放行）
  S2  6 个独立 fresh 进程 + matched auth + state_dir=None
      -> 零 winner（全部 BLOCKED，零 ALLOWED_AUTHORIZED_PAID）
  S3  6 个独立 fresh 进程 + 同一有效共享 state_dir + matched auth
      -> 恰好 1 个 ALLOWED_AUTHORIZED_PAID；其余 5 个 BLOCKED；
         marker 指纹正确、无授权原文；fresh replay 仍 BLOCKED
  S4  replay 到达真实 runner 边界（marker 已被 claim，同 auth 重新提交）
      -> WAITING + BLOCKED_COST_APPROVAL + authorization_consumed=true
         + hermes 未 spawn（loser/replay 不越过 invocation 边界）
...
用法：python tests/fresh_runner_cost_guard_fix005_validation.py
...
DEFAULT_EVIDENCE_ROOT = (
    ROOT / ".aaf" / "AAF-v0.5-A0-PAID-GUARD-001-FIX-005" / "fresh-runner-validation"
)
EVIDENCE_ROOT = Path(
    os.environ.get("AAF_FRESH_EVIDENCE_ROOT", "").strip() or DEFAULT_EVIDENCE_ROOT
).resolve()

TASK_ID = "AAF-FRESH-GUARD-005"
SCOPE = f"{TASK_ID}|hermes|deepseek-v4-flash|deepseek"
TASK_TEXT = f"""# Task ID
{TASK_ID}

# Task Name
fresh-runner fail-closed-without-persistent-state validation

# Objective
验证 v0.5 A0 Paid Guard FIX-005：无持久化 state 权威时 paid admission fail closed
...
def _run_worker_contention(state_dir_arg: str, n: int) -> dict:
    """n 个独立 python 进程各执行一次准入 claim（真实 resolve + env invocation-truth）。"""
    procs = [
        subprocess.Popen(
...
# ---------------------------------------------------------------------------
# 真实 runner（fresh process）+ fake bin
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# S1 / S2：state_dir=None fail closed（fresh 进程）
# ---------------------------------------------------------------------------


def _scenario_s1() -> tuple[bool, list[str]]:
    """S1：独立 fresh 进程 + matched paid auth + state_dir=None → BLOCKED。"""
    scenario_dir = EVIDENCE_ROOT / "S1-state-dir-none-blocked"
    if scenario_dir.exists():
        shutil.rmtree(scenario_dir)
    scenario_dir.mkdir(parents=True)
    failures: list[str] = []
    ...
    p = subprocess.run(
        [sys.executable, "-u", str(WORKER), "NONE", TASK_ID],
        cwd=str(ROOT), env=_worker_env(),
        capture_output=True, text=True, encoding="utf-8", timeout=180,
    )
    _check("blocked_rc", p.returncode == 3, f"rc={p.returncode} {p.stdout!r}")
    _check(
        "blocked_decision",
        f'"decision": "BLOCKED_COST_APPROVAL"' in p.stdout,
        p.stdout,
    )
    _check(
        "not_consumed",
        '"authorization_consumed": false' in p.stdout,
        p.stdout,
    )
    _check("no_allowed", '"ALLOWED_AUTHORIZED_PAID"' not in p.stdout, p.stdout)

    record = {
        "scenario": "S1-state-dir-none-blocked",
        "rc": p.returncode,
...
def _scenario_s2() -> tuple[bool, list[str]]:
    """S2：6 个独立 fresh 进程 + matched auth + state_dir=None → 零 winner。"""
    scenario_dir = EVIDENCE_ROOT / "S2-state-dir-none-zero-winners"
    if scenario_dir.exists():
        shutil.rmtree(scenario_dir)
    scenario_dir.mkdir(parents=True)
    failures: list[str] = []
    ...
    data = _run_worker_contention("NONE", n=6)
    winners = [r for r in data["results"] if r["rc"] == 0]
    losers = [r for r in data["results"] if r["rc"] == 3]
    _check("zero_winners", len(winners) == 0, f"winners={len(winners)} {data['results']}")
    _check("all_blocked", len(losers) == data["n"], f"losers={len(losers)}")
    ...
    record = {
        "scenario": "S2-state-dir-none-zero-winners",
        "n": data["n"],
...
# ---------------------------------------------------------------------------
# S3：有效共享 state_dir 进程并发 → 恰好一个 winner
# ---------------------------------------------------------------------------


def _scenario_s3() -> tuple[bool, list[str]]:
    scenario_dir = EVIDENCE_ROOT / "S3-shared-state-dir-one-winner"
    if scenario_dir.exists():
        shutil.rmtree(scenario_dir)
    scenario_dir.mkdir(parents=True)
    ...
    data = _run_worker_contention(str(state_dir), n=6)
    winners = [r for r in data["results"] if r["rc"] == 0]
    losers = [r for r in data["results"] if r["rc"] == 3]
    ...
    record = {
        "scenario": "S3-shared-state-dir-one-winner",
        "n": data["n"],
        "winner_count": len(winners),
        "loser_count": len(losers),
        "results": data["results"],
...
# ---------------------------------------------------------------------------
# S6：顺序 replay（driver 进程 = fresh 进程）
# ---------------------------------------------------------------------------


def _scenario_s6() -> tuple[bool, list[str]]:
    """S6：顺序 replay —— driver 进程内 claim 成功后再次 evaluate 同 auth → BLOCKED。"""
    scenario_dir = EVIDENCE_ROOT / "S6-sequential-replay"
    if scenario_dir.exists():
        shutil.rmtree(scenario_dir)
    scenario_dir.mkdir(parents=True)
    state_dir = scenario_dir / "shared"
    ...
    task6 = f"{TASK_ID}-S6"
    scope6 = f"{task6}|hermes|deepseek-v4-flash|deepseek"
    ok, err = cg._claim_auth(scope6, scope6, state_dir)
    _check("first_claim", ok, err)
    saved = {k: os.environ.get(k) for k in (cg.ENV_MODEL, cg.ENV_PROVIDER, cg.ENV_AUTH)}
    os.environ.update({**PAID_ENV, "AAF_COST_AUTH": scope6})
    try:
        rec2 = cg.evaluate(task6, "hermes", state_dir=state_dir)
    finally:
...
def main() -> int:
    ...
    failed = 0
    print("=" * 72)
    print("AAF-v0.5-A0 Paid Guard FIX-005 — fresh-runner validation (Run N+1)")
    ...
    scenarios = [
        ("S1-state-dir-none-blocked", _scenario_s1, {}),
        ("S2-state-dir-none-zero-winners", _scenario_s2, {}),
        ("S3-shared-state-dir-one-winner", _scenario_s3, {}),
        (
            "S4-replay-at-runner-boundary",
            _runner_scenario,
            {
                "sid": "S4-replay-at-runner-boundary",
        ...
        (
            "S5-local-free-allowed",
            _runner_scenario,
            {
                "sid": "S5-local-free-allowed",
                "env": LOCAL_ENV,
                "expect": {
                    "status": "SUCCESS",
