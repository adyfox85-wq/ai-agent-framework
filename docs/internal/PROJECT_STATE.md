...
- hotkey listener 偶发失活已实现自动自恢复（RW-012 = SOLVED for v0.4：Bridge 唯一 owner 有界 backoff，失败经 Tray/状态窗口可见）；
