...
> Last Updated: 2026-09-05（AAF-v0.5-UX-COST-VISIBILITY-IMPLEMENT-001-FIX-002 — **paid fallback evidence validation（Codex REQUEST_CHANGE 唯一剩余 blocker 收口）**：display 层不再把 fb_paid 存在当 paid invocation 证据——单点 gate 复用既有权威 validator（fallback_runtime.validate_paid_fallback_runtime_record，fail-closed；延迟 import、零重实现）——仅 schema-valid authoritative record（decision_kind + attempted=true + 全字段 + 授权/一致性不变量）才 PAID / USED_PAID / FAILED；{} / 缺字段 / wrong decision_kind / attempted=false / 矛盾 / schema 违例 -> fail-soft UNKNOWN 无 actual paid usage；ai_agent_framework/* 零修改（零 authority 变更）；truth rule 零变化；测试 = test_cost_visibility.py 63 项（FIX-002 Req 11 A–H + valid cases 保持）聚焦 116 passed + Bridge/status/recovery 173 passed + 分块全量 non-GUI 2241 passed · 1 skipped 零失败；记录 = docs/internal/AAF-v0.5-UX-COST-VISIBILITY-IMPLEMENT-001-FIX-002-REPORT.md；PRE_ALLOWED_UNTRACKED 保留；no push。此前更新：
> Last Updated: 2026-09-05（AAF-v0.5-UX-COST-VISIBILITY-IMPLEMENT-001-FIX-001-RESUME-001 — **FIX-001 timeout 恢复收口**：FIX-001 Hermes stage TimeoutExpired（3600.89s）未 commit；working tree 保留审计（assessment = PARTIALLY_FIXED，零丢弃零重做）；RESUME-001 独立复跑 全部测试（聚焦 102 passed / Bridge·status·recovery 173 passed / 分块全量 non-GUI 2198 passed · 1 skipped · 9 deselected 零失败——与原记录一致）+ REPORT §8 timeout recovery 记录；单一 recovery commit on cb52abd（未 amend 未 push）；route acceptance PENDING；FRESH_RUNNER_VALIDATION_REQUIRED 保持。此前更新：
> Last Updated: 2026-09-05（AAF-v0.5-UX-COST-VISIBILITY-IMPLEMENT-001-FIX-001 — **v0.5 Bridge Cost / Model Visibility truth 语义修复> Last Updated: 2026-09-05（AAF-v0.5-UX-COST-VISIBILITY-IMPLEMENT-001-FIX-001 — **v0.5 Bridge Cost / Model Visibility truth 语义修复（planned/authorized ≠ actual invocation；Codex REQUEST_CHANGE 唯一 blocker 收口）**：display layer 不再把 guard authorization / routing candidate evidence 显示成 actual invocation truth——actual Model/Provider/FREE/LOCAL_FREE/PAID/USED_* 只由 actual invocation 证据填充（<agent>_result.md valid 或 A5 真实执行审计）；guard ALLOWED_* / routing candidate 单独存在 -> Actual UNKNOWN + 显式 "Planned:" 标签（灰字区分 actual；CostRow.planned + status_window 渲染）；guard/routing model 绝不进入 actual；WorkBuddy/Codex 同一 truth rule；BLOCKED 仅权威 guard 阻断；gate AUTHORIZED 无执行证据不 USED_PAID；missing/corrupt fail-soft -> UNKNOWN；ai_agent_framework/* 零修改 （零 authority 变更）；frozen MVP / PH-1 CLOSED / PH-2·PH-3 NOT STARTED 边界保持；测试 = test_cost_visibility.py 49 项（FIX-001 Req 18 A–K）+ test_status_window.py 53 项（fixture 补真实 result.md 证据）——聚焦 102 passed；回归 = Bridge/status/recovery 173 passed + 分块全量 non-GUI 2198 passed / 1 skipped 零失败（RW-029 隔离）；记录 = docs/internal/AAF-v0.5-UX-COST-VISIBILITY-IMPLEMENT-001-FIX-001-REPORT.md；PRE_ALLOWED_UNTRACKED 保留；no push。此前更新：
...
> （ac6c489，route legs = WorkBuddy PASS_WITH_WARNING 非阻断 + Codex APPROVE）+ Gap Audit AAF-v0.5-PH1-
> ROLE-REPLACEMENT-GAP-AUDIT-001（PH1_RUNTIME_GAPS_NOT_BLOCKING——GAP-PH1-01/02/03 仍属实 non-blocking，
> 无 PH-1 runtime 实现任务）收口；记录 = Planner CONTRACT_REPLACEABLE_NOW（零 runtime 变更可替换、不依赖
> 旧 ChatGPT 历史）/ Executor·Validator·Reviewer REPLACEABLE_WITH_ADAPTER（Hermes/WorkBuddy/Codex = 当前
> 实现 only）；3 项 PORTABILITY_GAP 定义保留；PH-2 / PH-3 = NOT STARTED（未启动）；替换路径 = 契约 -> 单点
> adapter 编辑 -> 兼容验证 -> 既有 TASK/stage/REPORT 权威；frozen MVP intact；§7 Summary PH-001 行转
> CLOSED / COMPLETE；closure record = docs/internal/AAF-v0.5-PH1-PORTABILITY-CLOSE-001-REPORT.md；docs-only
> 零 runtime/code/test/registry/economics/qualification 变化；no push (TASK: AAF-v0.5-PH1-PORTABILITY-
> CLOSE-001)。此前更新：
> Last Updated: 2026-09-05（AAF-v0.5-PH1-PLANNER-BOOTSTRAP-ROLE-CONTRACTS-001 — **v0.5 PH-1 Portability
> Hardening = STARTED（docs-only deliverable 备妥；用户显式批准的 PH-1 scope；parent = 0c5dfad；final
> route acceptance = PENDING——WorkBuddy 独立验证 + Codex review 按 route 阶段执行）**：新增新 Planner
> 仓库入口 START_HERE_FOR_NEW_PLANNER.md + 正式角色契约 docs/internal/AAF_ROLE_CONTRACTS.md（四角色
> A/B/C/D + mapping + 分类证据 + 替换评估 + 最小替换流程 + PORTABILITY_GAP + PH-1 gap table；Planner =
> CONTRACT_REPLACEABLE_NOW / Executor/Validator/Reviewer = REPLACEABLE_WITH_ADAPTER；替换满足同一契约
> 不重定义 AAF；本任务零 runtime 实现）+ PH-1 record = docs/internal/AAF-v0.5-PH1-PLANNER-BOOTSTRAP-
> ROLE-CONTRACTS-001-REPORT.md；frozen MVP 保持 intact（不重开 A6/A4+、旧 roadmap 无 phase 自动激
> 活）；§7 Summary 新增 PH-001 行；docs-only 零 runtime/code/test/registry/economics/qualification
> 变化；no push (TASK: AAF-v0.5-PH1-PLANNER-BOOTSTRAP-ROLE-CONTRACTS-001)。此前更新：
> Last Updated: 2026-09-05（AAF-v0.5-MVP-FREEZE-CLOSE-001 — **v0.5 个人轻量 Agent 协作调度 MVP 正式冻结
（docs-only freeze closure record；final closure route acceptance = PENDING——WorkBuddy 独立验证 + Codex
closure 按 route 阶段执行）**：依 AAF-v0.5-MVP-BOUNDARY-EXIT-AUDIT-001（MVP_EXIT_READY / 无 MVP_MISSING /
A6 全 scope + A4+ = NOT_REQUIRED_FOR_MVP）收口——产品定义 / MVP 能力边界 sufficient / freeze criteria /
post-freeze opt-in policy / 角色契约抽象（Planner/Executor/Validator/Reviewer）+ concrete mappings
（ChatGPT/Hermes/WorkBuddy/Codex = replaceable implementations）成文于 PROJECT_STATE v0.5「MVP FROZEN」块
+ closure record docs/internal/AAF-v0.5-MVP-FREEZE-CLOSE-001-REPORT.md；non-MVP 扩张（A6 / A4+ /
long-term memory / self-learning / large DAG orchestration / agent marketplace / distributed workers /
queue / large dashboard-platform UI / plugin ecosystem / complex autonomous scheduling / complex
long-term model scoring / large-scale subagent system / Agent OS expansion / 图形化 Cost Gate UX）一律
NOT_REQUIRED_FOR_MVP、非自动 FUTURE phases——须显式用户 scope 批准；REQUIRED_BEFORE_A5_CLOSE 9 项 /
A0-A5 历史 / A6+A4+ 边界不变；docs-only 零 runtime/code/test/registry/economics/qualification 变化；
§7 Summary 新增 MVP-001 行；no push (TASK: AAF-v0.5-MVP-FREEZE-CLOSE-001)。此前更新：2026-09-05（AAF-v0.5-A5-CLOSURE-STATE-FINALIZE-001 — **A5 权威状态收口 = CLOSED / COMPLETE / SYNCED**（final closure route accepted——2026-09-05 依实际完成的 final WorkBuddy 独立 closure validation = AAF-v0.5-A5-CLOSE-WORKBUDDY-VALIDATION-002（SUCCESS / PASS_WITH_WARNING / blocking_rework = false）+ final Codex closure = AAF-v0.5-A5-CLOSE-CODEX-FINAL-001（SUCCESS / APPROVE）+ remote sync = AAF-v0.5-A5-CLOSE-SYNC-001（SUCCESS / local == origin/main == cd2e056 / ahead/behind = 0/0）accepted；Unresolved Issues = None——docs-only 收口，零 runtime/code/test/model-registry/economics/qualification 变化；本文件 CAP-003 / CAP-004 / RW-027 Related / §7 Summary 的 9 处 route-PENDING marker 收口为 final accepted + SYNCED；CLOSE-001 / PRECHECK-001 / FIX-001 时点历史保留（见下条与 PROJECT_STATE Last Updated）；REQUIRED_BEFORE_A5_CLOSE 9 项不重写；A6 / A4+ future scope 边界不变；A5-001..004 历史不改写；closure record = docs/internal/AAF-v0.5-A5-CLOSE-001-REPORT.md §8；no push (TASK: AAF-v0.5-A5-CLOSURE-STATE-FINALIZE-001)。此前更新：2026-09-05（AAF-v0.5-A5-CLOSE-001 — **A5 = CLOSED / COMPLETE**（2026-09-05 依已完成的 AAF-v0.5-A5-CLOSURE-AUDIT-001（判定 A5_READY_TO_CLOSE）正式关闭（closure record 备妥；final closure route acceptance = PENDING——WorkBuddy 独立 closure validation 因外部 429 未完成、Codex final closure 未执行，见 PROJECT_STATE「A5 CLOSED」块；2026-09-05 AAF-v0.5-A5-CLOSE-001-FIX-001 收口超前表述）——docs-only closure，零 runtime/code/test/model-registry/economics/qualification 变化）：REQUIRED_BEFORE_A5_CLOSE 9 项 closure requirements 全部由已接受的 A5-001（fallback decision + audit contract foundation，2026-09-02）/A5-002（FREE/LOCAL_FREE fallback runtime + FIX-001/FIX-002）/ A5-003（paid escalation Cost Gate + FIX-001/FIX-002）/ A5-004（authorized paid fallback runtime，AAF-v0.5-A5-PAID-FALLBACK-RUNTIME-001，2026-09-05）交付满足；closure boundary = PROJECT_STATE「A5 Scope Formalization」块（completion boundary / 9 项不重写）；A5-001..004 历史任务记录保持、不重写（A5 整体 closure 与历史实现单元记录区分保持）；A6（health scoring / quarantine / requalification / calibration）与 A4+（HIGH/CRITICAL WorkBuddy、Codex/multi-agent routing）保持 future scope NOT IMPLEMENTED、未进入；CAP-003 全套行 / RW-027 Related / CAP-004 Do Not Forget / §7 Summary CAP-003 行同步为 A5 = CLOSED / COMPLETE；closure record = docs/internal/AAF-v0.5-A5-CLOSE-001-REPORT.md；no push (TASK: AAF-v0.5-A5-CLOSE-001)。此前更新：2026-09-02（AAF-v0.5-A5-FALLBACK-CONTRACT-001 — **A5 = STARTED（第一条实现单元 = fallback decision + audit contract foundation 交付，NOT CLOSED / COMPLETE）**——`ai_agent_framework/fallback_contract.py` = 唯一权威 A5 fallback decision contract（bounded failure taxonomy 8 类 / 决策 4 token / one-fallback budget 编入 schema（max 1 per affected stage、count_used ∈ {0,1}、耗尽即拒 = 无 chain/loop）/ same-model transport retry（RW-027）分层分离不计入预算 / candidate 资格复用 A2 selector（A1 capability/qualification/main-scope 闸）不建平行系统 / authoritative audit record（Req 5 全字段 + explicit no-silent-fallback evidence）validate fail-closed；foundation 零 runtime 接线：fallback_attempted/fallback_used 恒 False、无第二模型执行、无 paid escalation、A3/A4 路由与 A0 Paid Guard 零变化）；131 项新增聚焦测试（tests/test_a5_fallback_contract.py）零回归；REQUIRED_BEFORE_A5_CLOSE 未全部满足（runtime 接线 / 自动 fallback 执行 / free→paid / Cost Gate UX 待后续单元），A5 未 CLOSED/COMPLETE；report = docs/internal/AAF-v0.5-A5-FALLBACK-CONTRACT-001-REPORT.md；no push (TASK: AAF-v0.5-A5-FALLBACK-CONTRACT-001)。此前更新：2026-09-02（AAF-v0.5-A3-HERMES-EXECUTOR-QUALIFICATION-FIX-001 — Prevent auxiliary-only models from Hermes active routing（HIGH-risk routing-authority fix，parent = A3 close line，A3 closure 不重开；A5 scope 文档零改动）：CAP-003 的 A3 Hermes slice 实现语义修正——**Hermes executor qualification 现要求 evidence 覆盖真实 main-chat/主调用路径（RuntimeQualification 新增 scope=main/auxiliary/unknown，缺省 unknown fail closed）**；真实自托管实证（LOW routing 曾选中 qwen3:4b@custom 且 routing_applied=true，但真实 `hermes -m qwen3:4b --provider custom` = HTTP 400）→ qwen3:4b@custom evidence 只覆盖 aux 槽位 + 本地端点 → scope=auxiliary → selector executor 角色以新 token AUXILIARY_ONLY 排除（unknown scope → MAIN_INVOCATION_UNPROVEN）；当前真实证据下 LOW Hermes 唯一 main-scope eligible = deepseek-v4-flash@deepseek 但 cost UNKNOWN 非 FREE → SELECTED_NOT_FREE → routing_applied=false、configured DeepSeek 保留、零 fallback、Paid Guard 零变化（LOW 无合格 free executor = 正确状态）；真正 main-scope 合格的 free executor 仍被路由（行为保持支持，专项 fresh-runner N2 实证）；16 项新增聚焦测试 + 既有 bug 语义测试翻转 + 全量 non-GUI 4-file-deselect 1832 passed / 1 skipped / 38 deselected（HEAD f6c577d 基线 1815 + 17 零回归）+ fresh-runner N+1 4/4（真实 facts no-route / aux-sole NO_ELIGIBLE / main-scope routed ALLOWED_FREE / routed-failure no-fallback WAITING）+ 既有 A3/A3-FIX-001/A4 fresh-runner 驱动期望同步后复跑全绿；A4 WB validator routing 零变化（WB 候选 scope=main，validator role 不经 executor scope 闸）；A4+ / A5（READY_TO_START 保持）/ A6 未进入；report = docs/internal/AAF-v0.5-A3-HERMES-EXECUTOR-QUALIFICATION-FIX-001-REPORT.md；no push (TASK: AAF-v0.5-A3-HERMES-EXECUTOR-QUALIFICATION-FIX-001)。此前更新：2026-09-02（AAF-v0.5-A5-SCOPE-FORMALIZATION-001 — Formalize A5 fallback / Cost Gate completion boundary（docs-only scope decision，零 runtime/code/test/model-registry 变化）：A5 正式 completion boundary = **bounded, auditable fallback / escalation / Cost Gate foundation**（REQUIRED_BEFORE_A5_CLOSE 9 项 closure requirements：fallback trigger/failure classification 明确（minimum failure classes = transport/runtime failure、 model unavailable/invocation failure、capability/qualification insufficiency、quality/validation failure、paid escalation required）/ 每 affected stage 最多 1 次 automatic model-level fallback / transport retry 与 model fallback 分开 / qualified candidate 存在时 free/local→free/local fallback / capability/qualification 先于 cost / 无安全 fallback fail closed / free→paid 必经现有 Paid Guard / Cost Guard authority（不建第二套付费授权）/ 不静默 跨越 role/risk/capability boundary / 全部 decision 可审计并保留最终实际模型 provenance）+ one-fallback rule（transport retry 不计入、无 chain/loop）+ free fallback rule（仅已 QUALIFIED 候选；FREE ≠ qualification）+ paid escalation rule（fail closed）+ audit artifact 字段 + A6/A4+ 显式 outside）→ **A5 = READY_TO_START**（非 CLOSED / COMPLETE，实现 NOT STARTED、未进入——前置 blocker = completion boundary 缺失已消除）；CAP-003 全套行（title / Status / Current Implementation 尾 / Remaining Gap / Do Not Forget）+ RW-027 Related / Remaining Gap + CAP-004 Remaining Gap / Do Not Forget + §7 Summary CAP-003 行同步（A5 completion boundary = PROJECT_STATE「A5 Scope Formalization」块；RW-027 transport retry ≠ A5 model-level fallback 分层分离记录；CAP-004 free-to-free fallback / 图形化 Cost Gate UX 正式归 A5 scope）；A5 实现未开始、A4+/A6 未进入、A0-A4 不重开；decision record = docs/internal/AAF-v0.5-A5-SCOPE-FORMALIZATION-001-REPORT.md；no push (TASK: AAF-v0.5-A5-SCOPE-FORMALIZATION-001)。此前更新：2026-09-02（AAF-v0.5-A4-CLOSE-001 — Close A4 WorkBuddy economic routing phase（docs-only closure，零 runtime/code/test/model-registry 变化）：**A4 = CLOSED / COMPLETE**（2026-09-02 依 AAF-v0.5-A4-SCOPE-FORMALIZATION-001 正式化的 closure boundary 关闭——closure boundary = WorkBuddy economic active routing foundation for LOW + MEDIUM risk，9 项 closure requirements 全部由已接受交付满足（A4-001 LOW + A4-002 MEDIUM，含 FIX-001），A4 formal scope 无 remaining gap；WorkBuddy 独立验证 + Codex APPROVE = route 阶段）；**A4+ future scope 保持 NOT IMPLEMENTED**（HIGH / CRITICAL WorkBuddy routing、multi-agent / Codex routing——prerequisite 记录保持，本任务不实现、不标记实现）；A5（fallback/escalation/Cost Gate UX）/ A6（health/quarantine/runtime requalification/observation/calibration）保持 NOT STARTED、未进入；CAP-003 全套行 / RW-027 Related / CAP-002 CI 尾句 / CAP-004 Do Not Forget / §7 Summary 同步更新为 A4 = CLOSED；closure record = docs/internal/AAF-v0.5-A4-CLOSE-001-REPORT.md；no push (TASK: AAF-v0.5-A4-CLOSE-001)。此前更新：2026-09-02（AAF-v0.5-A4-SCOPE-FORMALIZATION-001 — Formalize A4 completion boundary（docs-only scope decision，零 runtime/code/test/model-registry 变化）：**A4 正式 completion boundary = WorkBuddy economic active routing foundation for LOW + MEDIUM risk**——closure requirements（1 LOW WorkBuddy economic active routing implemented（A4-001）；2 MEDIUM WorkBuddy economic active routing implemented（A4-002）；3 capability/qualification 先于 economics；4 trustworthy economics fail closed（FRESH/完整/一致；STALE/UNKNOWN/incomplete/contradictory 不进经济排序）；5 >=2 economically trustworthy candidates required for active selection；6 per-run --model works when authority applies（恰好一个 --model，无 --effort/provider 覆盖）；7 otherwise CodeBuddy Auto（Auto 时不声称 routed_model）；8 no silent fallback（fallback_used 恒 false）；9 authoritative workbuddy_active_routing.json）已全部由已接受的 A4-001（LOW）+ A4-002（MEDIUM）交付满足 → **A4 = READY_TO_CLOSE**（非 CLOSED/COMPLETE/SYNCED——正式关闭 = 独立 Planner-approved closure task，按 A2/A3 惯例经 WorkBuddy 独立验证 + Codex APPROVE）；**A4+ future scope（NOT IMPLEMENTED，仅保留能力与 prerequisite 记录）**：HIGH WorkBuddy routing（前置 = T2-or-better WorkBuddy 候选 prerequisite qualification evidence 先于 active routing）、CRITICAL WorkBuddy routing（从未是 A4 closure requirement）、multi-agent / Codex routing（当前 authority 未定义 Codex candidate discovery / capability+qualification model / economic metadata source / per-run model-routing contract → 前置独立 scope/design 定义）；Hermes broader routing 在 A4 closure 之外（A3 不重开）；「future A4 work / A4-A6 / A4 = WorkBuddy/economic/multi-agent routing」歧义措辞已限定（消除 HIGH-required-before-A4-close / CRITICAL-implicit / Codex-in-A4-by-phase-name 误读）；A5（fallback/escalation/Cost Gate UX）/ A6（health/quarantine/runtime requalification/observation/calibration）边界不变；无功能删除、无未来能力放弃；decision record = docs/internal/AAF-v0.5-A4-SCOPE-FORMALIZATION-001-REPORT.md；no push (TASK: AAF-v0.5-A4-SCOPE-FORMALIZATION-001)。此前更新：2026-09-02（AAF-v0.5-A4-WORKBUDDY-ECONOMIC-ROUTING-002 — **MEDIUM WorkBuddy economic active routing 交付**（parent = AAF-v0.5-A4-WORKBUDDY-ECONOMIC-ROUTING-001-FIX-001，commit dfe0e01，未 amend 未 push；**A4 保持 STARTED**）：active-routing risk 域从 explicit LOW 扩展到 **explicit LOW + MEDIUM**——复用同一 workbuddy_routing 决策链（A2 selector / A1 registry+risk / A4 economic fact layer），不创建第二套 routing 路径；MEDIUM 复用既有 selector capability floor（RISK_FLOORS[MEDIUM].validator = T3，不单独放宽 eligibility，Req 5）；gate 顺序 / 经济学语义（FRESH/完整/一致/fail-closed）/ >=2 可信候选 gate / Auto fail-closed / artifact authority / no-fallback 契约原样复用（A4-001 + FIX-001）；HIGH/CRITICAL 在 active slice 之外 → 新 reason token `RISK_OUTSIDE_ACTIVE_SLICE`（A3 Hermes token 零改动）；当前真实 facts（真实 WorkBuddy 候选 T4 < MEDIUM floor T3 → 0 eligible）→ 真实 runtime N1 证明 Auto（INSUFFICIENT_ELIGIBLE_CANDIDATES / argv 精确 Auto 无 --model；Req 15 不伪造不放宽）；受控 MEDIUM 两可信 fixture（仅 wrapper 注入，production 零 hook）证明 routed 分支（N1b routing_applied=true / winner=med-free / argv 精确 "-p --output-format text -y --model med-free" 恰好一个 --model / artifact 与真实 invocation 一致）；LOW 行为零变化（N2a 真实 Auto / N2b 受控 routed hy4-preview）；21 项新增聚焦测试（tests/test_a4_workbuddy_routing_medium.py：Req 16 A–M 全矩阵 + floor 语义 + artifact authority + env + runner 集成）；全量 non-GUI 4-file-deselect **1815 passed / 1 skipped / 38 deselected** = stash 同命令 HEAD（dfe0e01）基线 **1794 + 21 精确零回归**；fresh-runner N+1 **6/6**（N1 MEDIUM 真实 Auto / N1b MEDIUM 受控 routed / N2a LOW 真实 Auto / N2b LOW 受控 routed / N3 HIGH control / N4 fresh-process no-silent-fallback + 回归 28/28）；既有 A4/A3 fresh-runner 驱动复跑全绿（A4-001 4/4、FIX-001 4/4、SECOND-CANDIDATE 3/3、QUALIFICATION 3/3、ECONOMICS 2/2、A3 2/2、A3-FIX-001 2/2；Hermes A3 / Codex 路由零修改，Req 17/18）；**A4 保持 STARTED**（LOW + MEDIUM WorkBuddy economic routing implemented；HIGH / broader multi-agent 仍未来 A4 工作，Req 19），A5/A6 未进入；report = docs/internal/AAF-v0.5-A4-WORKBUDDY-ECONOMIC-ROUTING-002-REPORT.md）。此前更新：2026-09-02（AAF-v0.5-A4-WORKBUDDY-ECONOMIC-ROUTING-001-FIX-001 — 两候选 economic routing gate 收口（Codex Requirement 5 blocker 完全消除；parent = AAF-v0.5-A4-WORKBUDDY-ECONOMIC-ROUTING-001，commit 6ca78d9，未 amend 未 push；**A4 保持 STARTED**）：最小候选数 gate 现在作用于 **capability sufficiency + qualification/usability + trustworthy economics 全部过滤之后**——只有 `len(economically_trustworthy) >= 2` 才允许 economic winner selection；经济过滤后只剩 1 个可信候选 → 新 reason `INSUFFICIENT_ECONOMIC_CANDIDATES`（显式区别于 capability 不足的 `INSUFFICIENT_ELIGIBLE_CANDIDATES`，Requirement 4 绝不混用）→ routing_applied=false / routed_model=None / fallback_used=false / CodeBuddy Auto；0 个 → 既有 `NO_TRUSTWORTHY_ECONOMIC_WINNER` 保持；当前真实 facts（deepseek-v4-flash freshness UNKNOWN 被经济排除 → 仅 hy4-preview 可信）→ **真实 runtime N1 证明 Auto**（不伪造第二可信候选，Req 13）；受控两可信候选 fixture（fixture/evidence injection，与真实 N1 明确区分）证明 active route 分支仍正确执行（Req 14：full-runner fresh-process N1b routing_applied=true / selected=hy4-preview / fake codebuddy argv 精确 "-p --output-format text -y --model hy4-preview" 恰好一个 --model / artifact 与真实 invocation 一致）；capability/qualification gate（Req 5）、economic trustworthiness 定义（FRESH/完整/一致/fail-closed，Req 6）、经济排序策略（Req 7）零变化；无 hard-code（generic 候选测试，Req 8）；no silent fallback 保持（fallback_used=false + transport retry 复用同一 routed args）；active artifact 准确记录 eligible / economically_trustworthy / routing_applied / routed_model / no-route reason / fallback_used=false（Req 11）；17 项新增聚焦测试（tests/test_a4_workbuddy_routing_fix001.py：Req 12 A–G 全矩阵（A 2-eligible-1-trustworthy Auto / B 2-eligible-2-trustworthy routed+恰好一个 --model / C 3-eligible-2-survive routed / D 2-eligible-0-survive Auto / E 1-eligible Auto / F STALE-UNKNOWN-incomplete-contradictory 不能凑满两候选 / G 输入顺序无关）+ reason token 区分 + 不硬编码 + artifact 准确性）+ 父测试文件 8 项 bug 语义测试翻转为新 gate（真实 facts → Auto；受控 fixture → routed）+ runner LOW 集成测试改受控 fixture 注入 = 定向 A4 聚焦 155 passed；全量 non-GUI 4-file-deselect **1794 passed / 1 skipped / 38 deselected** = stash 同命令 HEAD（6ca78d9）基线 **1777 + 17 精确零回归**；fresh-runner N+1 **4/4**（N1 LOW 真实 facts：routing_applied=false / routed_model=None / reason=INSUFFICIENT_ECONOMIC_CANDIDATES / fake codebuddy argv 精确 Auto 无 --model / 全链 SUCCESS；N1b LOW 受控两可信：routing_applied=true / argv 精确 routed / artifact 与 invocation 一致；N2 HIGH control Auto；N3 fresh-process real-facts Auto + 受控 routed 分支 + no-silent-fallback）；既有 4 个 A4 prerequisite fresh-runner 驱动（QUALIFICATION/SECOND-CANDIDATE/ECONOMICS/ECONOMICS-FIX-001）N1 LOW 期望同步回 Auto 形状后复跑全绿 + A3 / A3-FIX-001 驱动复跑全绿（Hermes 路由零变化）；**A4 保持 STARTED**（broader MEDIUM/HIGH/multi-agent 仍未来 A4 工作，NOT IMPLEMENTED），A5/A6 未进入；report = docs/internal/AAF-v0.5-A4-WORKBUDDY-ECONOMIC-ROUTING-001-FIX-001-REPORT.md）。此前更新：2026-09-02（AAF-v0.5-A4-WORKBUDDY-ECONOMIC-ROUTING-001 — **A4 = STARTED**（第一条 active-routing slice）：WorkBuddy validator stage 第一条最小 active economic routing 交付——新模块 `ai_agent_framework/workbuddy_routing.py`（authoritative WorkBuddy routing decision + artifact `workbuddy_active_routing.json` + `AAF_WORKBUDDY_MODEL` env 覆盖；**复用 A2 selector（capability+qualification）/ A1 registry+risk / A4 economic fact layer，不创建第二套 eligibility 系统**，Requirement 1/3）；激活条件（Requirement 2/4/5/6/11）：显式 Risk: LOW + 至少两个 eligible WorkBuddy candidates + economic facts FRESH/完整/一致/可审计（只消费 `workbuddy_economics.cheapness_rank` ∈ {RANK_AUTHORITATIVE_CHEAP, RANK_FRESH_DISCOUNT}；STALE/UNKNOWN/incomplete/contradictory fail closed）；生效时经济性更优的 qualified candidate 被**真实** per-run `--model` 执行（invocation 精确 [-p --output-format text -y --model <winner>]，恰好一个 --model；无 --effort/无 provider override/无 fallback/无 retry escalation，Requirement 9/10）；选择策略确定性 (cheapness_rank, multiplier, model_id)——当前 accepted facts 选中 **hy4-preview**（RANK_AUTHORITATIVE_CHEAP：FRESH+显式免费+multiplier 0.0+factor 0.0，窗口 2026-08-28..2026-09-11；generic 逻辑产出非硬编码，Requirement 8），deepseek-v4-flash（freshness UNKNOWN）经济排除但仍计入 eligible；**无 silent fallback**（fallback_used 恒 false，fixed semantic + validate fail-closed；transport retry 复用同一 routed args，Requirement 12）；artifact 与 A3 Hermes active_routing.json / A2 shadow_observation.json 明确区分（Requirement 13），Auto 保持时不声称路由（Requirement 14）；runner workbuddy stage 集成（invocation 前决策 + 原子写 artifact（写失败 fail closed）+ env apply/restore（observation 后还原）+ stage result workbuddy_active_routing_ref）；Hermes A3 路由零修改（Requirement 16）、Codex 零修改（Requirement 17）；事实层唯一路由消费方 = workbuddy_routing（A2/A3 权威 source-level 零直接引用保持）；34 项新增聚焦测试（tests/test_a4_workbuddy_routing.py：Req 15 全矩阵 / artifact 语义 / env / runner 集成）+ 既有 economics consumer 测试同步（定向 304 passed / 1 skipped）+ fresh-runner N+1 3/3（N1 LOW 真实进程 routing_applied=true / selected=hy4-preview / argv 精确 "-p --output-format text -y --model hy4-preview" / artifact 与真实 invocation 一致；N2 HIGH control routing_applied=false / argv 精确 Auto 无 --model / 全链 SUCCESS；N3 fresh-process no-silent-fallback）+ 既有 4 个 A4 prerequisite fresh-runner 驱动（QUALIFICATION/SECOND-CANDIDATE/ECONOMICS/ECONOMICS-FIX-001）N1 LOW 期望同步为 routed 形状后复跑全绿 + A3 驱动复跑全绿；**A4 = STARTED**（第一条 slice = LOW-risk WorkBuddy economic active routing），broader MEDIUM/HIGH/multi-agent 路由仍为未来 A4 工作（NOT IMPLEMENTED），A5（fallback/Cost Gate UX）/A6（observation/calibration/runtime requalification）未进入；A0-A3 = CLOSED/COMPLETE/SYNCED 不变；report = docs/internal/AAF-v0.5-A4-WORKBUDDY-ECONOMIC-ROUTING-001-REPORT.md）。此前更新：2026-09-02（AAF-v0.5-A4-PREREQ-WORKBUDDY-SECOND-CANDIDATE-001 — A4 economic routing 最后一个 prerequisite：qualify one economically relevant WorkBuddy candidate——**第二个资格化 WorkBuddy 候选 hy4-preview**（parent = AAF-v0.5-A4-PREREQ-WORKBUDDY-ECONOMICS-001-FIX-001，commit d7ab18a，未 amend 未 push）：经济事实层新增 fail-closed probe-priority 选择器 `workbuddy_economics.select_probe_candidate`（纯函数：RANK_AUTHORITATIVE_CHEAP 优先，rank-0 内最早 valid_until 确定性 tie-break；STALE/UNKNOWN/字段缺失矛盾永不优先；无可信候选 → NO_TRUSTWORTHY_SECOND_CANDIDATE）→ 从 14 个未资格化候选中确定性选出 hy4-preview（FRESH + 显式 free + multiplier 0.0 + promotion_factor 0.0，窗口 2026-08-28..2026-09-11；与 hy3 同为权威免费时免费窗口最早关闭者优先）；真实隔离 per-run CodeBuddy runtime probe（.aaf/AAF-v0.5-A4-PREREQ-WORKBUDDY-SECOND-CANDIDATE-001/probe/，observed_at=2026-09-02T03:01:44+08:00，codebuddy 2.141.0）：--version / --help 文档化 / config get model 前后均空（Auto 保持）/ 真实 invocation `--model hy4-preview` 完成受控 Risk: LOW validator-like task 精确产出预期 verdict 块（exit 0 / stderr 空 / 无协议错误）；registry 变更限定到该候选：capability_tier=T4（LOW probe 只证明最低 T4，不推断 T3/T2/T1/T0）+ qualification=QUALIFIED（evidence=本任务 probe artifacts，observed_at 真实）+ cost_class=UNKNOWN / locality=UNKNOWN / provider=None 保持（经济事实只决定 probe priority，永不赋予能力/资格，Requirement 11）；deepseek-v4-flash WorkBuddy 条目零变化（Requirement 9）；其余 13 候选保持 tier=None + qualification=unknown（Requirement 10）；production invocation 零修改（CodeBuddy Auto [-p --output-format text -y] 无 --model/--effort）；selector LOW 下现存在两个 eligible WorkBuddy candidates（selected 仍 deepseek-v4-flash——key 字典序 tie-break 不变），MEDIUM/HIGH/CRITICAL 仍 CAPABILITY_INSUFFICIENT；17 项新增聚焦测试（tests/test_a4_workbuddy_second_candidate.py）+ 既有 qualification/discovery/economics/registry 测试同步（定向 167 passed；全量 non-GUI 4-file-deselect 1743 passed / 1 skipped / 38 deselected = HEAD 1726 + 17 精确零回归）+ fresh-runner N+1 3/3（N1 LOW 全生命周期 SUCCESS + fake codebuddy argv 精确 Auto 无 --model；N2 HIGH control 同；N3 fresh-process 两独立 eligible LOW WorkBuddy candidates + 路由权威零变化；既有 QUALIFICATION-001 与 ECONOMICS-FIX-001 驱动复跑全绿）；**A4 保持 NOT STARTED**；report = docs/internal/AAF-v0.5-A4-PREREQ-WORKBUDDY-SECOND-CANDIDATE-001-REPORT.md）。此前更新：2026-09-02（AAF-v0.5-A4-PREREQ-WORKBUDDY-ECONOMICS-001-FIX-001 — A4 economic prerequisite 语义收紧（Codex REQUEST_CHANGE 两个 blocking findings 收口；parent = AAF-v0.5-A4-PREREQ-WORKBUDDY-ECONOMICS-001，commit 3aeb5a9，未 amend 未 push）：**blocking #1** multiplier=None + FRESH discount 不再获得已知便宜 rank（cheapness_rank 的 RANK_FRESH_DISCOUNT 现要求 economic_fields_consistent：multiplier 已知且有效 + promotion_factor 已知且有效 + status 与二者不矛盾；缺失/矛盾 → RANK_UNKNOWN_OR_STALE 永不进入已知经济排序）；**blocking #2** authoritative free 现要求 FRESH + free + multiplier==0.0 + promotion_factor==0.0 四者同立（免费促销带非零 factor 一律 fail closed）；free+nonzero multiplier / discount 内部矛盾（factor=0.0/1.0、multiplier=0.0）/ STALE / UNKNOWN / 字段缺失 全部 fail closed；EconomicFact 构造新增 multiplier_raw 必须能解析回 multiplier 的校验（无法解释的 raw 不得背书数值）；改动只限经济事实层 gate（ai_agent_framework/workbuddy_economics.py），不扩大经济策略；capability/qualification gate 先于 economics 保持（FRESH FREE hy3/hy4-preview 仍 ineligible；deepseek-v4-flash T4+QUALIFIED 不变）；production WorkBuddy invocation 零修改（CodeBuddy Auto [-p --output-format text -y] 无 --model/--effort）；economic_facts.json 重新生成且事实不变；27 项新增 adversarial 聚焦测试（tests/test_a4_workbuddy_economics_fix001.py）+ 全量 non-GUI 4-file-deselect 1726 passed / 1 skipped / 38 deselected = HEAD 1699 + 27 精确零回归；fresh-runner N+1 4/4（N1 LOW 全生命周期 SUCCESS + fake codebuddy argv 精确 Auto；N2 fresh-process artifact 可读 + 路由权威零变化；N3 fresh-process artifact 重新生成+再读；N4 fresh-process malformed/incomplete/contradictory facts fail closed）；A4 保持 NOT STARTED；report = docs/internal/AAF-v0.5-A4-PREREQ-WORKBUDDY-ECONOMICS-001-FIX-001-REPORT.md）。此前更新：2026-09-02（AAF-v0.5-A4-PREREQ-WORKBUDDY-ECONOMICS-001 — A4 prerequisite slice 3：Capture WorkBuddy economic metadata facts——真实只读 runtime probe 建立经济事实层（.aaf/AAF-v0.5-A4-PREREQ-WORKBUDDY-ECONOMICS-001/economic_probe/，observed_at=2026-09-02T02:10:30+08:00，codebuddy 2.141.0）：主源 WorkBuddy RemoteConfig 缓存 ~/.workbuddy/cache/acc-product-config-v3.json（ACC_PRODUCT_CONFIG_V3 / genieVersion 5.4.5 / date=2026-08-29T17:16:22.337Z / commit=fa1d65e）models[].credits 乘数 + modelPromotions[]（hy3/hy4-preview 限时免费 0x 带 validFrom/validUntil 窗口、glm-5.2/deepseek-v4-flash/-pro 夜间折扣 0.5x 仅 daily 时段）+ 佐证 CodeBuddy CLI catalog 缓存（local_storage，ts=2026-09-02T02:01:14+08:00；minimax-m2.7 x0.26 vs x0.19 差异记录在案）；15 候选全有乘数事实、5 个显式促销事实；新事实层模块 ai_agent_framework/workbuddy_economics.py（EconomicFact / parse_multiplier / classify_freshness FRESH-STALE-UNKNOWN fail closed / is_authoritative_cheap 需 FRESH+显式 free+multiplier 0.0 三者同立 / cheapness_rank 保证 STALE-UNKNOWN 永不 outrank 已知 FRESH）；经济事实只存储、零消费方（adapters/shadow_routing/active_routing/runner/cost_guard/model_registry 源码级零 import）；FRESH FREE 的 hy3/hy4-preview 仍 tier=None+qualification=unknown → ineligible（economics 低于 capability/qualification gate）；production WorkBuddy invocation 零修改（CodeBuddy Auto，[-p --output-format text -y] 无 --model/--effort）；29 项聚焦测试 + 全量 non-GUI 4-file-deselect 1699 passed / 1 skipped / 38 deselected = HEAD 1670 + 29 精确零回归；fresh-runner N+1 2/2（N1 LOW 全生命周期 SUCCESS + fake codebuddy argv 精确 Auto 无 --model/--effort；N2 fresh-process economic artifact 可读 + freshness 可复现 + 路由权威零变化）；A4 保持 NOT STARTED；report = docs/internal/AAF-v0.5-A4-PREREQ-WORKBUDDY-ECONOMICS-001-REPORT.md）。此前更新：2026-09-02（AAF-v0.5-A4-PREREQ-WORKBUDDY-QUALIFICATION-001 — A4 prerequisite slice 2：WorkBuddy 候选 deepseek-v4-flash 真实资格化——真实、隔离、可审计 per-run CodeBuddy runtime probe（.aaf/AAF-v0.5-A4-PREREQ-WORKBUDDY-QUALIFICATION-001/probe/，observed_at=2026-09-02T01:45:31+08:00，codebuddy 2.141.0）：`codebuddy --version`=2.141.0、`--help` --model 行仍文档化 deepseek-v4-flash（CLI 级接受）、`config get model` invocation 前后均空（CodeBuddy Auto 保持）、真实 invocation `codebuddy -p --output-format text -y --model deepseek-v4-flash --no-session-persistence` 完成受控 Risk: LOW validator-like task 并精确产出预期 verdict 块（exit 0 / stderr 空 / 无超时协议错误模型不可用 runtime failure）；registry 变更限定到该唯一候选：capability_tier=T4（LOW probe 只证明最低 T4，不推断 T3/T2/T1/T0）+ qualification=QUALIFIED（evidence=probe artifacts，observed_at 真实）+ cost_class=UNKNOWN / locality=UNKNOWN / provider=None 保持；其余 14 候选仍 tier=None + qualification=unknown；Hermes 同名条目（deepseek-v4-flash@deepseek）非本条目 qualification authority；production invocation 零修改（无 --model/--effort，CodeBuddy Auto）；selector LOW executor/validator 下通过 eligibility gate、MEDIUM/HIGH/CRITICAL 仍 CAPABILITY_INSUFFICIENT；16 项新增聚焦测试（parametrize 展开 18 项）+ 既有测试同步（定向 188 passed；全量 non-GUI 4-file-deselect 1668 passed / 1 skipped / 40 deselected = HEAD 1650 + 18 精确零回归）+ fresh-runner N+1 3/3（fake codebuddy argv 精确 Auto 无 --model）；**A4 保持 NOT STARTED**（正式实现 scope 未开始）；report = docs/internal/AAF-v0.5-A4-PREREQ-WORKBUDDY-QUALIFICATION-001-REPORT.md。此前更新：2026-09-02（AAF-v0.5-A4-PREREQ-WORKBUDDY-DISCOVERY-001 — A4 prerequisite slice 交付：WorkBuddy 具体候选身份事实层建立——真实只读 probe（.aaf/AAF-v0.5-A4-PREREQ-WORKBUDDY-DISCOVERY-001/discovery/，observed_at=2026-09-02T01:23:05+08:00）证明 codebuddy 2.141.0 `--help` --model 帮助行文档化当前支持的 15 个 model IDs（hy4-preview/hy3/hy3-x/glm-5.3/glm-5.3-flash/glm-5.2/glm-5.1/glm-5v-turbo/minimax-m3/minimax-m2.7/kimi-k3-1/kimi-k2.7/kimi-k2.6/deepseek-v4-pro/deepseek-v4-flash）；`config get model` 空（CodeBuddy Auto 保持）+ settings.json 无经济 key；15 个 identity-only 候选条目写入 model_registry baseline（capability_tier=None / qualification=unknown / cost_class=UNKNOWN / locality=unknown / provider=None 全保守；is_usable_candidate fail closed，发现 ≠ eligible）；selector 可枚举 WorkBuddy 候选但零 eligible（NO_SHADOW_CANDIDATE）；hermes/codex stage ROLE_NOT_APPLICABLE 零变化；hy4-preview-x 当前 runtime 已不列出 → 不收录（当前 runtime 不支持即不发明）；RemoteConfig 当前 runtime 不可观测 → 只登记源事实，不解析 multiplier/promotion；adapters 调用零修改（无 --model/--effort）；13 项新增聚焦测试 + 全量 non-GUI 4-file-deselect 1650 passed / 1 skipped / 40 deselected（HEAD 1637 + 13 精确零回归）；**A4 保持 NOT STARTED**（本 slice 只是 prerequisite 事实层，正式实现 scope 未开始）；report = docs/internal/AAF-v0.5-A4-PREREQ-WORKBUDDY-DISCOVERY-001-REPORT.md。此前更新：2026-09-01（AAF-v0.5-A3-CLOSE-001 — Close A3 Hermes Free Auto Routing：CAP-003 的 A3 slice（LOW-risk Hermes FREE active routing）= **IMPLEMENTED + CLOSED**（A3 = CLOSED / COMPLETE / SYNCED，依据已完成的 A3 Closure Audit 判定 A3_READY_TO_CLOSE：A3-001（2e41251）+ FIX-001（06fbaf1）= COMPLETE / APPROVED / SYNCED，A3 formal scope 内无 remaining gap）；**broader model routing / MEDIUM-HIGH / fallback / health / multi-agent 仍 NOT IMPLEMENTED，正式归入 A4-A6**（A4 = WorkBuddy/economic/multi-agent routing、A5 = fallback / Cost Gate UX、A6 = observation/calibration/runtime requalification），非 A3 remaining scope；docs-only，零 runtime/code/test/model registry 行为变化；closure record = docs/internal/AAF-v0.5-A3-CLOSE-001-REPORT.md。此前更新：2026-09-01（AAF-v0.5-A3-HERMES-FREE-ROUTING-001 — LOW-risk Hermes FREE active routing 首片交付（**A3 = STARTED**，未标 COMPLETE）：新模块 `ai_agent_framework/active_routing.py` = authoritative routing decision（**复用现有 A2 selector / A1 registry / risk 契约，不创建第二套路由判断**）；runner Hermes stage 在 Paid Guard 求值前决策（audit artifact = active_routing.json，authoritative=true，risk+provenance / considered / eligible / selected / routing_applied / routed+configured model+provider / reason / fallback_attempted=false；与 shadow_observation.json hypothetical 明确并存区分）；仅当 Hermes executor task 显式 Risk: LOW + selector eligible + selected 候选 cost_class ∈ FREE_OF_COST_CLASSES + qualification=QUALIFIED → routing_applied=true → 设置 AAF_HERMES_MODEL/PROVIDER/BASE_URL 覆盖（adapters 透传 -m/--provider；base_url = registry evidence 端点事实，A0 guard 经既有 classify_cost loopback 判定 LOCAL_FREE → ALLOWED_FREE 零授权零 claim——Paid Guard 未被绕过）；Risk 缺失（RISK_UNAVAILABLE）/ MEDIUM/HIGH/CRITICAL（RISK_NOT_LOW）/ 无候选（NO_SHADOW_CANDIDATE）/ 非 FREE（SELECTED_NOT_FREE）→ 保持 configured deepseek-v4-flash@deepseek；**无 silent fallback**（fallback_attempted 恒 false + validate fail-closed；invocation 失败如实 FRAMEWORK_ERROR → WAITING）；model_registry 新增 base_url 字段（qwen3:4b@custom = http://127.0.0.1:11434/v1，CAP-002/RW-030 证据）；cost_guard 新增 AAF_HERMES_BASE_URL 支持（仅显式端点事实，非 loopback 仍 fail-closed）；31 项新增聚焦测试（Req 10 全矩阵 + Paid Guard invariant + env apply/restore + runner 集成）+ 全量 non-GUI 零回归（stash 反证基线同命令 1595/1，本分支 1595 既有 + 31 新增 = 1626 全过）+ fresh-runner N+1 通过（N1 Risk: LOW 真实进程：routing_applied=true / guard ALLOWED_FREE+LOCAL_FREE / fake hermes 真实子进程可见 MODEL=qwen3:4b+PROVIDER=custom+BASE_URL=本地端点 / shadow actual_vs_shadow=SAME / 零 claim / 全链 SUCCESS；N2 Risk: HIGH control：routing 不生效、configured deepseek 原样、授权消费、全链 SUCCESS）；未实现 WorkBuddy/Codex routing / MEDIUM-HIGH 自动选择 / fallback / health polling / quarantine / promotion / A4-A6；A3 正式 COMPLETE 判定留待 WorkBuddy 独立验证 + Codex APPROVE + Planner。此前更新：2026-08-31（AAF-v0.5-A2-CLOSE-001 — Close A2 Shadow Routing：A2 正式状态 = CLOSED / COMPLETE / SYNCED（A2 Closure Audit 判定 A2_READY_TO_CLOSE；A2-001/002/003/004 + 相关 FIX 全部 APPROVED + SYNCED；formal scope 无 remaining gap，能力链 selector -> runtime shadow observation -> explicit Risk provenance -> evidence-backed eligible candidate 全部交付）；边界明确——runtime qualification observation = A2+（本条目 RW-030，非 A2 closure requirement、非「后续 A2 slice」，未实现）、actual routing = A3（CAP-003，NOT IMPLEMENTED，未进入）；authoritative=false / execution_affected=false 保持；A3-A6 未进入；docs-only，零行为变化；closure record = docs/internal/AAF-v0.5-A2-CLOSE-001-REPORT.md。此前更新：2026-08-31（AAF-v0.5-A2-SHADOW-ROUTING-004 — evidence-backed Hermes registry qualification：deepseek-v4-flash@deepseek 用已接受的 003-FIX-001 真实运行证据（Risk: HIGH 任务实际以该模型执行至 Codex APPROVE，commit 5911d39）填入 capability_tier=T2（HIGH executor floor，只证明至少 T2，不推断 T1/T0）+ qualification=QUALIFIED（accepted evidence snapshot：evidence 引用具体已接受 artifacts、observed_at=2026-08-31T08:16:23 真实接受时间戳；不表示永久健康、无动态 health/quarantine）；cost_class 保持 UNKNOWN；其它候选零改动（本地/free 模型无独立证据不提升）；selector / shadow observation 零代码修改；actual 执行不变；authoritative=false / execution_affected=false 保持；HIGH Hermes shadow 产生真实 hypothetical candidate（blocker 解除）；8 项新增聚焦测试 + 全量 non-GUI 零回归（1611+8）+ fresh-runner N+1 通过；A2 = STARTED（Shadow-only）。此前更新：2026-08-31（AAF-v0.5-A2-SHADOW-ROUTING-003 — explicit task-risk provenance：TASK contract 新增可选顶层 `Risk: LOW|MEDIUM|HIGH|CRITICAL`（唯一词汇 = risk_contract.RISK_CLASSES）；Planner 显式 Risk = structured provenance——缺失 → 向后兼容（shadow risk = RISK_UNAVAILABLE，missing ≠ LOW）、非法 → 双层校验严格拒绝；runner Hermes stage 从 immutable snapshot 解析 Risk 透传 shadow observation（risk_source = task/planner provenance）并允许现有 selector 产生真实 hypothetical decision；prose/name/route/path 零推断；authoritative=false / execution_affected=false 保持；A2 = STARTED（Shadow-only）。此前更新：2026-08-31（AAF-v0.5-A2-SHADOW-ROUTING-001 — A2 Shadow Routing 首片交付：deterministic shadow-selection engine（`ai_agent_framework/shadow_routing.py`，纯函数、零 I/O/LLM）——假设性 / 非权威 / 零执行影响（live 模块零 import，隔离测试锁定）；消费 A1 Registry + Risk 契约原样复用（tier_satisfies / is_usable_candidate / FREE_OF_COST_CLASSES / floor_for / reviewer_allowed）；能力充分性先于成本、FREE ≠ qualification、UNKNOWN 成本保守规则（已知成本 > UNKNOWN）、显式 NO_SHADOW_CANDIDATE、输入顺序无关确定性；reviewer 语义与 schema_version 严格性零回归；47 项定向测试 + 全量 non-GUI 零回归；A2 = STARTED（未 COMPLETE）；CAP-003（实际路由）仍 NOT IMPLEMENTED。此前更新：2026-08-31（AAF-v0.5-A1-CLOSURE-PROTOCOL-CORRECTION-001 — structured-result 协议纠正 + A1 closure 复核：只读诊断从仓库实现确认 structured-result 架构（producer = `adapters._structured_contract_block` 注入 Hermes prompt；Hermes raw schema = status/commit/changed_files/warnings，**无 commit_changed**；归一化权威 = `<agent>_result.json`，commit/commit_changed/changed_files 全部 Framework git 观察派生）；正式政策新增 AAF_TASK_EXECUTION_POLICY §5.2（raw vs normalized authority hierarchy + corrected acceptance rules：不得要求抑制 Framework 必输出块、raw 缺非契约字段 ≠ provenance failure、冲突时 Framework 归一化事实权威）；历史 FIX-003/FIX-004 artifacts 与 verdict 不可变不重写（TASK 层协议假设有误，reviewer 判断无错）；closure-loop 教训登记 RW-032；A1 在纠正后的验收语义下复核无新 blocker → **A1 = CLOSED / COMPLETE 保持**，Next mainline = A2 Shadow Routing 不变；文档-only 提交，未 push（review 后同步））。此前更新：2026-08-30（AAF-v0.5-A1-CLOSURE-AUDIT-001-FIX-001 — A1/A2 边界裁决 + A1 正式关闭：四项曾列 "A1 remaining slices"（Selection Engine / Shadow Routing / Hermes candidate tier 赋值 / 运行时 qualification 观测）经逐项裁决全部归类 A2+——Shadow Routing = A2（显式）、运行时 qualification 观测 = A2+（显式，见本文件 RW-030）、Selection Engine / Hermes candidate tier 赋值 = A2+（推断）；A1 正式 scope（Registry + Risk 契约，见 RW-030 Current Implementation）无剩余 blocker → **A1 = CLOSED / COMPLETE**，Next mainline = v0.5 -> A2 Shadow Routing（未启动）；PROJECT_STATE / A1 REPORT / CAP-004 已同步裁决；零代码修改、无 push（review 后同步））。此前更新：2026-08-30（AAF-v0.5-RUNTIME-UX-CONSOLE-FLASH-001 — Windows helper 子进程 console 闪现修复收口：确认的 git/helper/model-observation flash 来源（`context_packet.py` 6 处 git 调用、`git_status._git()`、`model_observation._run_readonly()`）统一复用既有 `no_console_kwargs()`（CREATE_NO_WINDOW），子进程语义零变化（13+/0-）；`status_window.py` explorer 经检查不改（GUI-subsystem 无 console 路径）；10 项聚焦测试 + 定向 443 passed + 全量 non-GUI 零回归 + fresh-runner Run N+1 通过；登记 backlog **RW-031** = CLOSED（原始证据级别：confirmed Windows subprocess console-visibility omission，cosmetic/UX only，非功能正确性失败）；未 push（review 后同步）；返回点 = v0.5 -> A1 Registry + Risk）。此前更新：2026-08-30（AAF-v0.5-A1-REGISTRY-RISK-001 — A1 Registry + Risk foundation 交付：新增 `ai_agent_framework/model_registry.py`（model registry 契约：身份 / 适用 agent / capability tier / cost 分类（复用 model_observation.COST_CLASSES）/ locality / runtime qualification 独立维度；`is_usable_candidate` 仅由 tier + qualification 判定，FREE 绝不隐含 qualified）+ `ai_agent_framework/risk_contract.py`（LOW/MEDIUM/HIGH/CRITICAL + 初始 tier-floor 映射 + 自托管权威区域至少 HIGH + 分类纯逻辑零 LLM）；Hermes FREE-model 可用性/稳定性用户观察登记为本文件 **RW-030**（user-observed runtime constraint，非普遍断言、非永久健康结论）；A1 = STARTED（foundation slice delivered，remaining planned slices 未完成，A1 未 full closed）；基线 registry 只填证据事实，未验证一律 UNKNOWN；无自动路由激活、无 A2-A6 实现、cost_guard/runner/router 零修改；测试 58 项新增 + 全量 non-GUI 1453 passed / 1 skipped 零回归）。此前更新：2026-08-30（AAF-v0.5-A0-PAID-GUARD-001-CLOSE-001 — A0 Paid Guard 正式关闭：FIX-006 = APPROVED / COMPLETE（Codex APPROVE，blocking_rework=false，blocking_provenance=structured，commit b1e8bf2）；A0 = CLOSED / COMPLETE；工作树对账——tracked working tree CLEAN（0 tracked 修改），「dirty」观察 = 3 个 PRE_ALLOWED_UNTRACKED untracked 常驻项（.aaf/、AAF_TASK004_PROCESS_CHECK.txt、scripts/start_bridge_hidden.vbs），未删除未 clean；Next mainline = A1 Registry + Risk，A1 未启动；Hermes free-model 可用性/稳定性为后续路由的 runtime qualification concern，不假定 FREE=healthy；closure 变更仅文档（A0 REPORT §13 / PROJECT_STATE / 本文件 CAP-004），cost_guard 零修改、未 push（ahead 6/0，review 通过后同步）。此前更新：2026-08-30（AAF-v0.5-A0-PAID-GUARD-001-FIX-006 — Codex BLOCKING（FIX-005 review）最终收口：持久化 state_dir fail-open class——`_claim_auth` 只拒绝 None、`state_dir=""` 经 `Path("")` 解析为相对 CWD 的 marker（不同 CWD 独立进程各自可 winner）→ 新增 `_state_dir_validation_error()`：paid admission 的持久化 state_dir 必须为**显式提供的、非空的、绝对路径**，None / 空串 / 纯空白 / `Path("")` CWD fallback / `"."` / 相对路径 / 畸形（NUL）/ 类型非法一律 fail closed 且**零 marker 创建（含 CWD）**；校验先于一切消费检查与文件系统写入；mkdir/open 兜底 `(OSError, ValueError)`；FIX-003 原子性 / FIX-002 端点安全 / runner 恒传绝对 output_dir / LOCAL_FREE 全部保持；新增 24 项聚焦测试（空/空白/Path("")/./相对/bytes/int/NUL blocked + CWD 零 marker、6 进程不同 CWD invalid state 零 winner + 各 CWD 零写入、共享绝对 state_dir 恰一 winner、顺序 replay、绝对路径是文件/中间组件是文件/marker 被目录占用 fail closed、runner 级相对 output_dir → WAITING + Hermes 零 spawn + 无 marker、LOCAL_FREE 不受影响、FIX-002 endpoint 抽样）+ fresh-runner Run N+1 7/7（S1–S7，含真实 runner 相对 output 边界零 spawn）；既有驱动复跑 FIX-005 6/6、FIX-003 5/5、FIX-002 9/9 保持绿色；定向 106 passed、分块全量 non-GUI 1395 passed / 1 skipped 零回归（RW-029 flake 惯例隔离）。此前更新：2026-08-30（AAF-v0.5-A0-PAID-GUARD-001-FIX-005 — Codex BLOCKING（FIX-003 review）最终收口：paid admission 无持久化 filesystem authority → fail closed——`_claim_auth(state_dir=None)` 直接失败（`_STATE_DIR_REQUIRED_ERR`：filesystem exclusive-create 是唯一跨进程准入权威，`_CONSUMED_AUTHS` 永远只是非权威拒绝快路径、绝不放行）；paid admission 序列 = 精确匹配 → 有效持久化 state_dir → 原子 exclusive-create claim → ALLOWED_AUTHORIZED_PAID，任何前置失败 → BLOCKED；FIX-003 原子性 / FIX-002 端点安全全部保持；移除 3 项「state_dir=None 可产生一个 paid winner」旧断言，新增 17 项 A–H 聚焦测试（单次 / 8 线程 / 6 独立进程 state_dir=None → 零 winner、共享 state_dir 恰一 winner、顺序 replay、state_dir 是文件 / marker 被目录占用 / runner 级 persistence 不确定 fail closed + Hermes 零 spawn、LOCAL_FREE 保持、paid 无授权 blocked、FIX-002 endpoint 对抗抽样）+ fresh-runner Run N+1 6/6，FIX-003 驱动 5/5、FIX-002 驱动 9/9 复跑保持绿色；定向 82 passed、分块全量 non-GUI 复跑零回归。此前更新：2026-08-30（AAF-v0.5-A0-PAID-GUARD-001-FIX-003 — Codex BLOCKING #3 最终收口：授权消费原子化——check-then-consume 移除，`cost_guard._claim_auth()` 单一原子操作（filesystem exclusive-create `open(...,"x")` 为跨进程权威，marker 已占用 / 无法持久化 → fail closed；`_CONSUMED_AUTHS` 降级为非权威只拒绝快路径 + `_CONSUMED_LOCK`；消费记录不再落盘授权原文，sha256 指纹即可等值判定）；13 项并发/对抗测试（线程 barrier 8 线程恰 1 winner、6 独立进程恰 1 claim 成功 + fresh replay blocked、顺序 replay、不同 scope 不消费、marker 被目录占用 / state_dir 是文件 fail closed、旧格式 marker 兼容拒绝、runner 级 replay 零 Hermes 调用）+ fresh-runner Run N+1 5/5（进程 contention 1 winner / 5 blocked、replay 达真实 runner 边界零 spawn、LOCAL_FREE 放行、paid 无授权 blocked、顺序 replay）；定向 65 passed、分块全量 1354 passed / 0 failed（单进程连跑 RW-029 0x80000003 既有 flake 隔离复跑通过）。此前更新：2026-08-29（AAF-v0.5-A0-PAID-GUARD-001-FIX-002 — Codex REQUEST_CHANGE 三个 blocking fail-open 修复：本地端点判定改 hostname/IP 严格语义（substring 匹配移除，localhost.evil.example / path-embedded 127.0.0.1 / 0.0.0.0 等一律 fail closed）；`AAF_COST_FREE_MODELS` 不再作为权威 FREE 来源（A0 无远程 FREE registry，设置即忽略并记录诊断 note）；`AAF_COST_AUTH` 改为准入即消费的真一次性（in-process 集合 + 执行目录 `cost_auth_consumed.json`，replay 拒绝、消费状态不确定 fail closed）；新增对抗回归测试（test_cost_guard_fix002.py）+ fresh-runner S7/S8/S9 对抗场景。此前更新：2026-08-29（AAF-v0.5-A0-PAID-GUARD-001 — v0.5 A0 Hermes Paid Guard 交付：fail-closed task-scoped 成本授权——Hermes stage subprocess 创建前 guard（`ai_agent_framework/cost_guard.py`）：effective model 解析（`AAF_HERMES_MODEL`/`AAF_HERMES_PROVIDER` env 优先 + `hermes config get model` 只读 fallback；零网络/零 LLM）、A0 最小成本分类（LOCAL_FREE / 仅显式元数据的 FREE / 其余 PAID_OR_UNKNOWN / 无法解析 COST_UNKNOWN fail closed）、一次性 task-scoped 授权（`AAF_COST_AUTH="<Task ID>|<stage>|<model>[|<provider>]"` 整串精确匹配，env per-run 不泄漏）、机器可读决策（ALLOWED_FREE / ALLOWED_AUTHORIZED_PAID / BLOCKED_COST_APPROVAL，`cost_guard.json` 含 decision_ms）；blocked → Hermes 进程零创建 + 任务 WAITING（COST_APPROVAL_REQUIRED）+ 人读 blocked 信息完整（Task ID/stage/model/provider/cost/原因/所需 scope）；Hermes 全局 config 零修改；`AAF_HERMES_MODEL`/`AAF_HERMES_PROVIDER` 透传 `-m`/`--provider`（无覆盖时 args 与 v0.4 一致）；34 项定向测试 + 全量 non-GUI 1323 passed（1289 基线 + 34 新增，零下降；RW-029 0x80000003 环境 flake 隔离复跑通过）+ fresh-runner Run N+1 6/6 场景（blocked 零 spawn / local-free 达 invocation 边界 / 精确授权放行 / 其他 Task 与 model 授权阻断 / unresolved fail closed）；CAP-004 = IMPLEMENTED (A0)；完整 Model Routing（CAP-003）仍 NOT IMPLEMENTED（A1+ 范围））
...
| Remaining Gap | NONE（transport retry 层完成）。注意：自动切换模型 / 更贵模型 / Cost Gate / Model Routing 均 NOT IMPLEMENTED（属未来 Model Routing 阶段，本任务明确禁止）。A5 正式 boundary（2026-09-02 AAF-v0.5-A5-SCOPE-FORMALIZATION-001）确认：本层 retry = transport execution 层（同 invocation 同模型），**不是** A5 model-level fallback、也不构成 A5 实现依据（A5 的 automatic fallback = 每 affected stage 最多 1 次换模型 fallback，另见 PROJECT_STATE「A5 Scope Formalization」块） |
...
| Related | RW-007（Agent executable discovery reliability——MISSING_COMMAND 仍永久失败不重试）；CAP-003（Future Model Routing——A3 LOW-risk Hermes FREE active routing = IMPLEMENTED + CLOSED；A4 LOW + MEDIUM WorkBuddy economic active routing = IMPLEMENTED + CLOSED / COMPLETE（A4 = CLOSED——2026-09-02 AAF-v0.5-A4-CLOSE-001 正式关闭）；HIGH / CRITICAL WorkBuddy 与 multi-agent / Codex routing = A4+ future scope NOT IMPLEMENTED；其余 broader model routing 属 A4-A6；A5（fallback / escalation / Cost Gate foundation）= CLOSED / COMPLETE / SYNCED（2026-09-05 AAF-v0.5-A5-CLOSE-001 依 AAF-v0.5-A5-CLOSURE-AUDIT-001（判定 A5_READY_TO_CLOSE）正式关闭（closure record 已接受——2026-09-05 AAF-v0.5-A5-CLOSE-001（786729e）+ FIX-001（cd2e056）经 final WorkBuddy 独立 closure validation（AAF-v0.5-A5-CLOSE-WORKBUDDY-VALIDATION-002 = SUCCESS / PASS_WITH_WARNING / blocking_rework = false）+ final Codex closure（AAF-v0.5-A5-CLOSE-CODEX-FINAL-001 = SUCCESS / APPROVE）accepted、由 AAF-v0.5-A5-CLOSE-SYNC-001 同步 origin/main（SYNCED / cd2e056 / 0/0）——2026-09-05 AAF-v0.5-A5-CLOSURE-STATE-FINALIZE-001 收口，见 PROJECT_STATE「A5 CLOSED」块；FIX-001 时点 route = PENDING / external 429 / precheck REQUEST_CHANGE = 历史时点记录）——REQUIRED_BEFORE_A5_CLOSE 9 项全部满足；completion boundary = 2026-09-02 AAF-v0.5-A5-SCOPE-FORMALIZATION-001 正式化保持（当时 READY_TO_START 为历史状态记录）——本 RW-027 transport retry（同一 invocation 同模型重试）与 A5 model-level fallback 分层分离：existing same-model transport retry 不计入 A5 每 stage 1 次 model fallback 预算）） |
...
| Remaining Gap | 其余本地/free 候选（qwen2.5vl:3b 等）的实况 qualification 观测待后续 A2+ slice 填充；当前无健康轮询、无自动隔离、无 fallback（均属 A2+ / A4-A6 future phases，非 A3 remaining scope）；actual routing = A3（CAP-003）已交付并关闭（LOW-risk Hermes FREE active routing = IMPLEMENTED / CLOSED / COMPLETE / SYNCED，2026-09-01 AAF-v0.5-A3-CLOSE-001）——FREE ≠ healthy 纪律不因 A3 关闭而改变 |
| Decision | 登记为 OBSERVATION；路由层（A2 已关闭的 shadow 消费 / A3 已关闭的 LOW active routing / A4-A6 未来路由）消费 registry 时必须把 FREE（成本维度）与 qualification（运行时维度）分开：FREE ≠ healthy，UNKNOWN ≠ free |
...
| Current Implementation | 每 agent 独立 discover_*：Hermes 主模型 + auxiliary slots（本地 Ollama 可发现）；CodeBuddy 当前模型 UNKNOWN（config 不暴露）+ CLI help 文档化模型 ID——2026-09-02 AAF-v0.5-A4-PREREQ-WORKBUDDY-DISCOVERY-001 起，当前 CLI（2.141.0）--model 帮助行文档化的 15 个 model IDs 已以 identity-only 条目写入 model_registry baseline（provenance：.aaf/AAF-v0.5-A4-PREREQ-WORKBUDDY-DISCOVERY-001/discovery/，observed_at=2026-09-02T01:23:05+08:00；capability_tier=None / qualification=unknown / cost_class=UNKNOWN / locality=unknown 全保守，发现 ≠ eligible；agent:workbuddy Auto 锚点保留）；**2026-09-02 AAF-v0.5-A4-PREREQ-WORKBUDDY-QUALIFICATION-001 起，deepseek-v4-flash 以真实 per-run probe 证据资格化**（真实 invocation `codebuddy -p --output-format text -y --model deepseek-v4-flash --no-session-persistence` 完成受控 Risk: LOW validator-like task，evidence = .aaf/AAF-v0.5-A4-PREREQ-WORKBUDDY-QUALIFICATION-001/probe/，observed_at=2026-09-02T01:45:31+08:00）→ capability_tier=T4（LOW probe 只证明最低 T4，不推断 T3/T2/T1/T0）+ qualification=QUALIFIED + cost_class=UNKNOWN / locality=UNKNOWN / provider=None 保持；其余 14 候选仍 identity-only（tier=None + qualification=unknown）；Hermes 同名条目（deepseek-v4-flash@deepseek）非 WorkBuddy qualification authority（独立 runtime evidence）；资格仅影响候选 eligibility（LOW selector 可选中），不影响任何实际 routing authority（production invocation 仍 CodeBuddy Auto，无 --model/--effort）；**2026-09-02 AAF-v0.5-A4-PREREQ-WORKBUDDY-SECOND-CANDIDATE-001 起，第二个 WorkBuddy 候选 hy4-preview 以独立真实 per-run probe 证据资格化**（candidate 由经济事实层 fail-closed probe-priority 选择器 workbuddy_economics.select_probe_candidate 从 14 个未资格化候选中确定性选出：FRESH + 显式 free + multiplier 0.0 + promotion_factor 0.0（窗口 2026-08-28..2026-09-11，RANK_AUTHORITATIVE_CHEAP），与 hy3 同为权威免费时免费窗口最早关闭者优先；STALE/UNKNOWN/字段缺失矛盾永不优先；选择只决定「先验证谁」，不是 routing authority，经济事实永不赋予能力/资格；evidence = .aaf/AAF-v0.5-A4-PREREQ-WORKBUDDY-SECOND-CANDIDATE-001/probe/，observed_at=2026-09-02T03:01:44+08:00）→ capability_tier=T4 + qualification=QUALIFIED + cost_class=UNKNOWN / locality=UNKNOWN / provider=None 保持；deepseek-v4-flash WorkBuddy 条目零变化；其余 13 候选仍 identity-only；selector LOW 下现存在两个 eligible WorkBuddy candidates（selected 仍 deepseek-v4-flash，key 字典序 tie-break），MEDIUM/HIGH/CRITICAL 仍 CAPABILITY_INSUFFICIENT；production invocation 零修改（CodeBuddy Auto）；**2026-09-02 AAF-v0.5-A4-PREREQ-WORKBUDDY-ECONOMICS-001 起，WorkBuddy 经济性元数据事实层建立**（provenance：.aaf/AAF-v0.5-A4-PREREQ-WORKBUDDY-ECONOMICS-001/economic_probe/，observed_at=2026-09-02T02:10:30+08:00）——主源 WorkBuddy RemoteConfig 缓存 acc-product-config-v3.json（ACC_PRODUCT_CONFIG_V3 / genieVersion 5.4.5 / date=2026-08-29）models[].credits 乘数（15 CLI 候选全有值）+ modelPromotions[] 促销事实（hy3/hy4-preview 限时免费 0x 带 validFrom/validUntil 窗口；glm-5.2/deepseek-v4-flash/-pro 夜间折扣 0.5x 仅 daily 时段，无日期窗口 → freshness=UNKNOWN fail closed）+ 佐证 CLI catalog 缓存（minimax-m2.7 x0.26 vs x0.19 差异记录在案）；新模块 workbuddy_economics.py（EconomicFact / classify_freshness FRESH-STALE-UNKNOWN / is_authoritative_cheap 需 FRESH+显式 free+multiplier 0.0 三者同立 / cheapness_rank 保证 STALE-UNKNOWN 永不 outrank 已知 FRESH）——只存储、零消费方（routing 代码源码级零 import）；FRESH FREE 的 hy3/hy4-preview 仍 tier=None+qualification=unknown → ineligible（economics 低于 capability/qualification gate）；production invocation 零修改；29 项聚焦测试 + 全量 non-GUI 1699 passed / 1 skipped / 38 deselected（HEAD 1670 + 29 精确零回归）；**2026-09-02 FIX-001（AAF-v0.5-A4-PREREQ-WORKBUDDY-ECONOMICS-001-FIX-001）语义收紧（Codex REQUEST_CHANGE 两个 blocking findings 收口）**：multiplier=None + FRESH discount 不再获得已知便宜 rank（cheapness_rank 的 RANK_FRESH_DISCOUNT 现要求 economic_fields_consistent：multiplier 已知且有效 + promotion_factor 已知且有效 + status 与二者不矛盾）；authoritative free 现要求 FRESH + free + multiplier==0.0 + promotion_factor==0.0 四者同立；free+nonzero multiplier / discount 内部矛盾 / STALE / UNKNOWN / 字段缺失 全部 fail closed（永不进入已知经济排序）；EconomicFact 构造新增 multiplier_raw 必须能解析回 multiplier 的校验；改动只限经济事实层 gate，不扩大经济策略；capability/qualification precedence 保持；production invocation 零修改；27 项新增 adversarial 聚焦测试（tests/test_a4_workbuddy_economics_fix001.py）+ 全量 non-GUI 1726 passed / 1 skipped / 38 deselected（HEAD 3aeb5a9 基线 1699 + 27 精确零回归）+ fresh-runner N+1 4/4（N1 LOW 全生命周期 SUCCESS + fake codebuddy argv 精确 Auto；N2 fresh-process artifact 可读 + 路由权威零变化；N3 fresh-process artifact 重新生成+再读；N4 fresh-process malformed/incomplete/contradictory facts fail closed）；**2026-09-02 AAF-v0.5-A4-WORKBUDDY-ECONOMIC-ROUTING-001 起，WorkBuddy validator stage 第一条 active economic routing 生效（A4 = STARTED）**：`ai_agent_framework/workbuddy_routing.py` = WorkBuddy authoritative active-routing decision（artifact `workbuddy_active_routing.json` + `AAF_WORKBUDDY_MODEL` env 覆盖；**复用 A2 selector / A1 registry+risk / A4 economic fact layer，不创建第二套 eligibility 系统**）——仅当显式 Risk: LOW + 至少两个 eligible WorkBuddy candidates + economic facts FRESH/完整/一致/可审计（cheapness_rank ∈ {0,1}；STALE/UNKNOWN/incomplete/contradictory fail closed）时，经济性更优的 qualified candidate 被真实 per-run `--model` 执行（invocation 精确 `[-p --output-format text -y --model <winner>]`，恰好一个 --model；无 --effort/provider override/fallback/retry escalation）；选择策略确定性 (cheapness_rank, multiplier, model_id)——当前 accepted facts 选中 hy4-preview（RANK_AUTHORITATIVE_CHEAP，FRESH+显式免费+multiplier 0.0+factor 0.0，窗口 2026-08-28..2026-09-11；generic 逻辑非硬编码），deepseek-v4-flash（freshness UNKNOWN）经济排除但仍计入 eligible；无 silent fallback（fallback_used 恒 false）；runner workbuddy stage 集成（决策+原子写 artifact+env apply/restore+stage ref），Hermes A3/Codex 路由零修改；34 项新增聚焦测试 + 既有 consumer 测试同步（定向 304 passed/1 skipped）+ fresh-runner N+1 3/3（N1 LOW 真实 --model hy4-preview 且 artifact 匹配实际 invocation / N2 HIGH Auto / N3 no-silent-fallback）+ 既有 4 个 A4 prerequisite fresh-runner 驱动 N1 期望同步复跑全绿；**A4 = CLOSED / COMPLETE（第一条 slice = LOW-risk WorkBuddy economic active routing（A4-001）+ MEDIUM 于 A4-002 实现，均 closure capability——2026-09-02 AAF-v0.5-A4-CLOSE-001 正式关闭）**；HIGH/CRITICAL/multi-agent = A4+ future scope（NOT IMPLEMENTED，closure boundary 见 PROJECT_STATE「A4 Scope Formalization」块），A5/A6 未进入；Codex 默认模型 server-side 不可枚举（documented limitation）+ `-m/--model` 显式选择能力；**2026-09-02 AAF-v0.5-A4-WORKBUDDY-ECONOMIC-ROUTING-002 起，active-routing risk 域 = explicit LOW + MEDIUM**：MEDIUM 复用既有 selector capability floor（RISK_FLOORS[MEDIUM].validator = T3）——当前真实 WorkBuddy 候选 capability_tier=T4 < T3 → MEDIUM 场景如实 Auto（0 eligible，INSUFFICIENT_ELIGIBLE_CANDIDATES），真实数据不被人为放宽/伪造；routing 决策只作用于 selector 已 eligible 候选（经济事实永不赋予 capability/qualification）；21 项新增聚焦测试（tests/test_a4_workbuddy_routing_medium.py：Req 16 A–M 全矩阵）+ 全量 non-GUI 1815 passed（HEAD 1794 + 21 零回归）+ fresh-runner N+1 6/6 |
...
## CAP-003 — Future Model Routing（能力登记：A3 LOW-risk Hermes FREE active routing = IMPLEMENTED + CLOSED；A4 LOW + MEDIUM WorkBuddy economic active routing = IMPLEMENTED + CLOSED / COMPLETE（A4 = CLOSED——2026-09-02 AAF-v0.5-A4-CLOSE-001 正式关闭，closure boundary 见 PROJECT_STATE.md v0.5「A4 Scope Formalization」块）；HIGH / CRITICAL WorkBuddy 与 multi-agent / Codex routing = A4+ future scope NOT IMPLEMENTED；其余 broader model routing 属 A4-A6 NOT IMPLEMENTED；A5（fallback / escalation / Cost Gate foundation）= STARTED（2026-09-02 AAF-v0.5-A5-FALLBACK-CONTRACT-001 交付第一条实现单元 = fallback decision + audit contract foundation + 2026-09-02 AAF-v0.5-A5-FREE-FALLBACK-RUNTIME-001 runtime 单元（bounded FREE/LOCAL_FREE automatic fallback 接入 Hermes executor live path；2026-09-03 FIX-001 收口两个 Codex runtime-contract blockers：ALLOWED_AUTHORIZED_PAID 不进入 FREE fallback invocation + authoritative audit closure 失败时不接受 fallback 输出）＋FIX-002（2026-09-03）收口 Codex 唯一 blocker：post-invocation audit closure 异常边界 exception-safe fail-closed：_emit catch 从 (ValueError, TypeError, OSError) 拓宽为 Exception + 兜底边界（RuntimeError/UnicodeError 等未预期 audit 异常统一收口、不裸逃逸；6 项新聚焦 + fresh-runner FIX-002 4/4）；**2026-09-03 AAF-v0.5-A5-PAID-ESCALATION-GATE-001（paid escalation Cost Gate runtime foundation 交付，A5 保持 STARTED / NOT CLOSED / COMPLETE）**：fallback-eligible 失败 + 无合格 FREE/LOCAL_FREE fallback + 合格 paid candidate（A1/A2 过闸、distinct from original、budget 未耗尽）→ 以既有 A0 Paid Guard（唯一付费授权 authority）做显式可审计授权判断——gate decision AUTHORIZED / BLOCKED / FAIL_CLOSED（exact task-scoped AAF_COST_AUTH；A0 一次性 claim 语义在准入边界保持，authorization_present/matched/consumed 如实转述）；**authorization-evaluation ONLY——零 paid invocation**（fallback_attempted/used 恒 False；AUTHORIZED 只建立 future paid invocation 资格）；权威审计 artifact paid_escalation_gate.json（decision_kind=paid_escalation_gate_audit，validate fail-closed，no-silent-paid-execution evidence）；A5-002 FREE fallback / A0 / contract 零修改，free candidate 优先于 paid escalation；全量回归零失败 + fresh-runner 5/5 场景（no auth / mismatch / exact auth / no-silent-paid / FREE intact）＋FIX-001（2026-09-03，parent 32c4bbe）收口 Codex 唯一 blocker：malformed/contradictory A0 authorization fail-closed 后仍生成 validator-valid、可持久化的自洽权威 paid_escalation_gate.json（source_guard_record raw/source 证据分层；14 项新聚焦 + canonical 2115 passed 零回归 + fresh-runner FIX-001 5/5；report = docs/internal/AAF-v0.5-A5-PAID-ESCALATION-GATE-001-FIX-001-REPORT.md）＋FIX-002（2026-09-03，parent 4c2ebf9，新 fix commit 未 amend 未 push）收口 Codex 唯一 blocker：exact paid authorization scope——ALLOWED_AUTHORIZED_PAID 的 required_scope 必须**精确等于** canonical expected scope（既有 A0 scope authority `cost_guard.scope_string(task_id, stage, model, provider)`；gate 以与 A0 evaluate 同源的 task_id/stage 重建 expected scope，单一 scope 格式不建第二套）；wrong task/stage/model/provider 或 malformed scope → FAIL_CLOSED（scope mismatch；raw 矛盾证据只在 source_guard_record 可观察，normalized 恒自洽）；validator 从 record 自身字段重建 canonical scope 独立拒绝手造 out-of-canonical-scope AUTHORIZED record（防御纵深零削弱）；exact canonical scope 行为零变化（AUTHORIZED 仍零 paid invocation）；8 项新聚焦（52→60）+ canonical 4-file-deselect 2101 passed 零回归（HEAD 4c2ebf9 基线 2093）+ fresh-runner FIX-002 5/5（wrong task / wrong stage → FAIL_CLOSED 落盘 + real A0 exact auth 与脚本化 canonical exact scope → AUTHORIZED 零 invocation + FREE intact）；report = docs/internal/AAF-v0.5-A5-PAID-ESCALATION-GATE-001-FIX-002-REPORT.md）；**A5 = CLOSED / COMPLETE / SYNCED**（2026-09-05 AAF-v0.5-A5-CLOSE-001 依 AAF-v0.5-A5-CLOSURE-AUDIT-001（判定 A5_READY_TO_CLOSE）正式关闭（closure record 已接受——2026-09-05 AAF-v0.5-A5-CLOSE-001（786729e）+ FIX-001（cd2e056）经 final WorkBuddy 独立 closure validation（AAF-v0.5-A5-CLOSE-WORKBUDDY-VALIDATION-002 = SUCCESS / PASS_WITH_WARNING / blocking_rework = false）+ final Codex closure（AAF-v0.5-A5-CLOSE-CODEX-FINAL-001 = SUCCESS / APPROVE）accepted、由 AAF-v0.5-A5-CLOSE-SYNC-001 同步 origin/main（SYNCED / cd2e056 / 0/0）——2026-09-05 AAF-v0.5-A5-CLOSURE-STATE-FINALIZE-001 收口，见 PROJECT_STATE「A5 CLOSED」块；FIX-001 时点 route = PENDING / external 429 / precheck REQUEST_CHANGE = 历史时点记录）——REQUIRED_BEFORE_A5_CLOSE 9 项全部满足；A5-004 = AAF-v0.5-A5-PAID-FALLBACK-RUNTIME-001（2026-09-05）已交付；上文 A5 = STARTED / NOT CLOSED 叙述 = 各实现单元任务时点的历史状态记录；closure record = docs/internal/AAF-v0.5-A5-CLOSE-001-REPORT.md）**——completion boundary 见 PROJECT_STATE.md v0.5「A5 Scope Formalization」块）；A6 = NOT STARTED）
...
| Status | **IMPLEMENTED（A3 slice：LOW-risk Hermes FREE active routing；A4 slice：LOW + MEDIUM WorkBuddy economic active routing——A4 = CLOSED / COMPLETE（2026-09-02 AAF-v0.5-A4-CLOSE-001））/ NOT IMPLEMENTED（A4+ future scope：HIGH / CRITICAL WorkBuddy routing、multi-agent / Codex routing；其余 broader model routing 属 A4-A6；A5（fallback / escalation / Cost Gate foundation）= STARTED（2026-09-02 AAF-v0.5-A5-FALLBACK-CONTRACT-001 foundation 单元 = fallback decision + audit contract 已交付 + 2026-09-02 AAF-v0.5-A5-FREE-FALLBACK-RUNTIME-001 runtime 单元（bounded FREE/LOCAL_FREE automatic fallback 已接入 Hermes executor live path；A5 保持 STARTED / NOT CLOSED——free→paid / Cost Gate UX 未开始）；FIX-001（2026-09-03）收口两个 Codex runtime-contract blockers：ALLOWED_AUTHORIZED_PAID 不进入 FREE fallback invocation（仅 ALLOWED_FREE 可执行；A0 Paid Guard 零修改）+ authoritative audit closure 失败时不接受 fallback 输出（attempted=true / used=false fail closed；FIX-002（2026-09-03）收口 Codex 唯一 blocker：post-invocation audit closure 异常边界 exception-safe fail-closed：_emit catch 从 (ValueError, TypeError, OSError) 拓宽为 Exception + 兜底边界（RuntimeError/UnicodeError 等未预期 audit 异常统一收口、不裸逃逸；attempted=true / used=false / 输出不被接受 / audit_closure_error surface / 无第三模型 / env 还原；6 项新聚焦 + fresh-runner FIX-002 4/4）；**2026-09-03 AAF-v0.5-A5-PAID-ESCALATION-GATE-001（paid escalation Cost Gate runtime foundation 交付，A5 保持 STARTED / NOT CLOSED / COMPLETE）**：fallback-eligible 失败 + 无合格 FREE/LOCAL_FREE fallback + 合格 paid candidate（A1/A2 过闸、distinct from original、budget 未耗尽）→ 以既有 A0 Paid Guard（唯一付费授权 authority）做显式可审计授权判断——gate decision AUTHORIZED / BLOCKED / FAIL_CLOSED（exact task-scoped AAF_COST_AUTH；A0 一次性 claim 语义在准入边界保持，authorization_present/matched/consumed 如实转述）；**authorization-evaluation ONLY——零 paid invocation**（fallback_attempted/used 恒 False；AUTHORIZED 只建立 future paid invocation 资格）；权威审计 artifact paid_escalation_gate.json（decision_kind=paid_escalation_gate_audit，validate fail-closed，no-silent-paid-execution evidence）；A5-002 FREE fallback / A0 / contract 零修改，free candidate 优先于 paid escalation；全量回归零失败 + fresh-runner 5/5 场景（no auth / mismatch / exact auth / no-silent-paid / FREE intact）＋FIX-001（2026-09-03，parent 32c4bbe）收口 Codex 唯一 blocker：malformed/contradictory A0 authorization fail-closed 后仍生成 validator-valid、可持久化的自洽权威 paid_escalation_gate.json（source_guard_record raw/source 证据分层；14 项新聚焦 + canonical 2115 passed 零回归 + fresh-runner FIX-001 5/5；report = docs/internal/AAF-v0.5-A5-PAID-ESCALATION-GATE-001-FIX-001-REPORT.md）＋FIX-002（2026-09-03，parent 4c2ebf9，新 fix commit 未 amend 未 push）收口 Codex 唯一 blocker：exact paid authorization scope——ALLOWED_AUTHORIZED_PAID 的 required_scope 必须**精确等于** canonical expected scope（既有 A0 scope authority `cost_guard.scope_string(task_id, stage, model, provider)`；gate 以与 A0 evaluate 同源的 task_id/stage 重建 expected scope，单一 scope 格式不建第二套）；wrong task/stage/model/provider 或 malformed scope → FAIL_CLOSED（scope mismatch；raw 矛盾证据只在 source_guard_record 可观察，normalized 恒自洽）；validator 从 record 自身字段重建 canonical scope 独立拒绝手造 out-of-canonical-scope AUTHORIZED record（防御纵深零削弱）；exact canonical scope 行为零变化（AUTHORIZED 仍零 paid invocation）；8 项新聚焦（52→60）+ canonical 4-file-deselect 2101 passed 零回归（HEAD 4c2ebf9 基线 2093）+ fresh-runner FIX-002 5/5（wrong task / wrong stage → FAIL_CLOSED 落盘 + real A0 exact auth 与脚本化 canonical exact scope → AUTHORIZED 零 invocation + FREE intact）；report = docs/internal/AAF-v0.5-A5-PAID-ESCALATION-GATE-001-FIX-002-REPORT.md）；修复细节见 PROJECT_STATE A5 块）；**A5 = CLOSED / COMPLETE / SYNCED**（2026-09-05 AAF-v0.5-A5-CLOSE-001 依 AAF-v0.5-A5-CLOSURE-AUDIT-001（判定 A5_READY_TO_CLOSE）正式关闭（closure record 已接受——2026-09-05 AAF-v0.5-A5-CLOSE-001（786729e）+ FIX-001（cd2e056）经 final WorkBuddy 独立 closure validation（AAF-v0.5-A5-CLOSE-WORKBUDDY-VALIDATION-002 = SUCCESS / PASS_WITH_WARNING / blocking_rework = false）+ final Codex closure（AAF-v0.5-A5-CLOSE-CODEX-FINAL-001 = SUCCESS / APPROVE）accepted、由 AAF-v0.5-A5-CLOSE-SYNC-001 同步 origin/main（SYNCED / cd2e056 / 0/0）——2026-09-05 AAF-v0.5-A5-CLOSURE-STATE-FINALIZE-001 收口，见 PROJECT_STATE「A5 CLOSED」块；FIX-001 时点 route = PENDING / external 429 / precheck REQUEST_CHANGE = 历史时点记录）——REQUIRED_BEFORE_A5_CLOSE 9 项全部满足；A5-004 = AAF-v0.5-A5-PAID-FALLBACK-RUNTIME-001（2026-09-05）已交付；上文 NOT CLOSED / COMPLETE 叙述 = 各实现单元任务时点的历史状态记录）**；completion boundary = 2026-09-02 AAF-v0.5-A5-SCOPE-FORMALIZATION-001 正式化）；A6 = NOT STARTED）** |
| Priority | P2 |
| Evidence / Origin | AAF-v0.4-TASK-010 Requirement 23：只登记未来 policy，不实现 |
| Current Implementation | A3 actual routing（LOW-risk Hermes FREE active routing slice）= **IMPLEMENTED + CLOSED / COMPLETE / SYNCED**——2026-09-01 AAF-v0.5-A3-HERMES-FREE-ROUTING-001 交付（A3-001 = 2e41251）+ FIX-001（06fbaf1，FREE_PROMO 严格排除）APPROVED + SYNCED，2026-09-01 AAF-v0.5-A3-CLOSE-001 依已完成的 A3 Closure Audit（A3_READY_TO_CLOSE）正式关闭（A3 formal scope 内无 remaining gap）：`ai_agent_framework/active_routing.py`（authoritative routing decision，**复用 A2 selector / A1 registry / risk 契约，不创建第二套路由判断**）+ runner Hermes stage 集成（Paid Guard 求值前决策 + AAF_HERMES_MODEL/PROVIDER/BASE_URL 覆盖，observation 后还原）+ cost_guard `AAF_HERMES_BASE_URL` 端点事实支持 + model_registry base_url 字段；仅当 Hermes executor task 显式 Risk: LOW + selector eligible + selected 候选 FREE/LOCAL_FREE + QUALIFIED → routing_applied=true → 真实 invocation 指向 qwen3:4b@custom（-m/--provider 透传，guard ALLOWED_FREE 零授权——Paid Guard 未被绕过）；Risk 缺失 / MEDIUM/HIGH/CRITICAL / 无候选 / 非 FREE（含 FREE_PROMO）→ 保持 configured deepseek-v4-flash@deepseek；无 silent fallback（fallback_attempted 恒 false）；审计 = active_routing.json（authoritative=true）与 shadow_observation.json（authoritative=false）并存区分；31 + 6 项聚焦测试 + 全量 non-GUI 零回归（stash 反证 1595/1 基线 + 31 新增 = 1626 全过；FIX-001 1631 + 6 = 1637 全过）+ fresh-runner N+1 通过（N1 LOW 真实路由到 qwen3 / N2 HIGH control 保持 deepseek）；**broader model routing 仍 NOT IMPLEMENTED，属 A4-A6（非 A3 remaining scope）**：MEDIUM/HIGH 自动模型选择（A4-A6）、automatic fallback（A5）、health polling / quarantine（A6）、automatic qualification promotion（A6）、WorkBuddy/economic/multi-agent routing（A4，当时均未实现——A4 的 WorkBuddy economic LOW/MEDIUM slice 已由 A4-001/FIX-001/-002 实现，multi-agent 经 AAF-v0.5-A4-SCOPE-FORMALIZATION-001 划归 A4+ future scope，见本行尾部）。注（2026-08-31，AAF-v0.5-A2-SHADOW-ROUTING-001）：A2 Shadow Routing 已 STARTED——`ai_agent_framework/shadow_routing.py` deterministic **shadow-selection engine**（假设性 / 非权威 / 零执行影响，live 路径零 import）；A2-002 接入 Hermes shadow observation（observation-only bypass）；A2-003 建立 explicit task-risk provenance（TASK 可选 `Risk` 字段 → immutable snapshot → shadow observation，Planner 显式结构化声明，缺失 = RISK_UNAVAILABLE，非法值严格拒绝）；A2-004 为 deepseek-v4-flash@deepseek 填入证据支持的 registry qualification（capability_tier=T2 + QUALIFIED，仅来自已接受的 003-FIX-001 HIGH 执行/审查证据，accepted evidence snapshot、不推断 T1/T0、cost_class 仍 UNKNOWN）——HIGH Hermes shadow 现产生真实 hypothetical candidate；本 CAP-003 指**实际** Model Routing（切换 / fallback / 升级），A2 阶段仍 NOT IMPLEMENTED；**2026-09-02 AAF-v0.5-A4-WORKBUDDY-ECONOMIC-ROUTING-001 + FIX-001 + -002 起，A4 WorkBuddy economic active routing slice 已部分实现**：WorkBuddy validator stage 在 explicit Risk: LOW + MEDIUM（002）且 >=2 capability-qualified + >=2 economically-trustworthy 候选时真实 per-run `--model` 经济路由生效（复用 A2 selector / A1 registry+risk / A4 economic fact layer；无 silent fallback；fallback_used 恒 false）；**2026-09-02 AAF-v0.5-A4-SCOPE-FORMALIZATION-001（docs-only）正式界定 A4 completion boundary**：A4 = WorkBuddy economic active routing foundation for LOW + MEDIUM risk（A4-001 LOW + A4-002 MEDIUM 满足全部 closure requirements——capability/qualification 先于 economics / trustworthy economics fail closed / >=2 economically trustworthy gate / per-run --model when authority applies / otherwise CodeBuddy Auto / no silent fallback / authoritative workbuddy_active_routing.json）→ **A4 = CLOSED / COMPLETE**（Formalization 时点 = READY_TO_CLOSE；正式关闭已于 2026-09-02 由 AAF-v0.5-A4-CLOSE-001 docs-only 执行——closure boundary / 9 项 requirements 保持本界定不变，A4 formal scope 无 remaining gap）；**HIGH / CRITICAL WorkBuddy routing（HIGH 前置 = T2-or-better WorkBuddy 候选 qualification evidence；CRITICAL 从未是 A4 closure requirement）与 multi-agent / Codex routing（前置 = 独立 scope/design：candidate discovery / capability-qualification model / economic metadata source / per-run model-routing contract）= A4+ future scope NOT IMPLEMENTED（A4 关闭不使其实现）**；automatic fallback / Cost Gate UX（A5）、health-polling-quarantine-runtime-requalification（A6）边界不变；**A5 = READY_TO_START（2026-09-02 AAF-v0.5-A5-SCOPE-FORMALIZATION-001 — docs-only 正式化 A5 completion boundary：bounded auditable fallback / escalation / Cost Gate foundation + REQUIRED_BEFORE_A5_CLOSE 9 项 closure requirements——one-fallback rule（每 affected stage 最多 1 次 model fallback、existing same-model transport retry（RW-027）不计入、无 chain/loop）、free fallback 仅限已 QUALIFIED 候选（FREE ≠ qualification）、 free→paid 必经现有 task-scoped Paid Guard / Cost Guard authority（fail closed，不建第二套付费授权系统）、 authoritative audit artifact 含 final actual model/provider + no-silent-fallback evidence——A5 实现 NOT STARTED、未进入；A4+ / A6 边界保持）**；**2026-09-02 AAF-v0.5-A3-HERMES-EXECUTOR-QUALIFICATION-FIX-001（HIGH-risk routing-authority fix，A3 slice 内修正实现，A3 closure 历史不重写 / A4-A6 边界不重开）：Hermes executor qualification 语义收紧——真实自托管实证 LOW routing 曾选中 qwen3:4b@custom（routing_applied=true）但真实 main-chat invocation `hermes -m qwen3:4b --provider custom` = HTTP 400（aux 模型 ≠ 主 API 模型；base_url Markdown 假设已证伪）→ 根因 = qualification 契约缺 evidence 调用上下文维度 → model_registry `RuntimeQualification.scope`（main / auxiliary / unknown，缺省 unknown fail closed）+ 基线 scope 事实（deepseek-v4-flash@deepseek=main；qwen3:4b@custom=auxiliary——RW-030 probe 只覆盖 aux 槽位/本地端点；WB 两资格化候选=main）+ shadow_routing executor 角色主调用资格闸（AUXILIARY_ONLY / MAIN_INVOCATION_UNPROVEN 排除 token；capability/qualification/cost gate 顺序强度零减弱；validator/reviewer hypothetical 零变化）→ A3/runner/cost_guard/adapters 生产逻辑零改动，同一 selector 语义自动修正：当前真实证据 LOW Hermes qwen3 不再 eligible、唯一 main-scope eligible = deepseek 但 cost UNKNOWN 非 FREE → SELECTED_NOT_FREE → routing_applied=false、configured DeepSeek 保留、零 fallback、Paid Guard 零变化（LOW 无合格 free executor = 正确状态；真正 main-scope 合格 free executor 仍可被路由——受控 fresh-runner N2 实证 ALLOWED_FREE 全链）；16 项新增聚焦回归（tests/test_a3_executor_qualification_fix.py）+ 既有 bug 语义测试翻转（test_shadow_routing/shadow_observation/active_routing/task_risk_provenance/a4 discovery+qualification 同步）+ 全量 non-GUI 4-file-deselect **1832 passed / 1 skipped / 38 deselected**（HEAD f6c577d 基线 1815 + 17 精确零回归）+ fresh-runner N+1 4/4（N1 真实 facts no-route+AUXILIARY_ONLY 审计 / N1b aux-sole NO_ELIGIBLE / N2 main-scope routed ALLOWED_FREE / N3 routed-failure no-fallback WAITING）+ 既有 A3/A3-FIX-001/A4 全系 fresh-runner 驱动期望同步复跑全绿（A4 LOW 场景 hermes stage 走精确 AAF_COST_AUTH = configured deepseek 保留）；A4 WB validator routing 零变化（WB scope=main + validator 不经 executor scope 闸）；report = docs/internal/AAF-v0.5-A3-HERMES-EXECUTOR-QUALIFICATION-FIX-001-REPORT.md。**2026-09-02 AAF-v0.5-A5-FALLBACK-CONTRACT-001（A5 第一条实现单元 = fallback decision + audit contract foundation → A5 = STARTED / NOT CLOSED / COMPLETE）**：`ai_agent_framework/fallback_contract.py` = 唯一权威 A5 fallback decision contract（纯逻辑、零 runtime 接线）——bounded failure taxonomy 8 类（Req 2 五类 + A5 scope minimum classes 余项）/ 决策 4 token（fallback_eligible / fallback_not_eligible / paid_escalation_required / blocked_fail_closed）/ one-fallback rule 编入 schema（budget = 1、count_used ∈ {0,1}、耗尽即拒 → 无 chain/loop）/ transport retry（RW-027）分层分离（transport_retry_count 独立字段、不计入预算、不翻转 attempted/used）/ 候选资格复用 A2 selector（select_shadow_candidate → A1 capability/qualification/executor main-scope 闸，不建平行 routing/cost/qualification 系统）/ cost-authorization blocked 与 paid escalation required 类 → 非 fallback 上下文或必经既有 A0 Paid Guard / Cost Guard authority（无第二付费授权系统）/ authoritative machine-readable audit record（task/stage/role/risk、original model-provider、failure class+trigger、eligibility、candidates、attempted/used、paid escalation、authorization outcome、final actual model-provider、explicit no-silent-fallback evidence）validate fail-closed；foundation fixed semantic：fallback_attempted / fallback_used 恒 False（validate 拒绝任何 True）、final actual == original、零第二模型执行、零 paid fallback 实现；131 项新增聚焦测试（tests/test_a5_fallback_contract.py：Req 8 全矩阵——每 failure class / eligible vs not-eligible / same-model retry 不计为 fallback / malformed fail-closed / audit schema required fields / attempted-used 恒 False 跨全场景）+ 全量 non-GUI 零回归；A3/A4 路由与 A0 Paid Guard 行为零变化（模块未被任何 live 路径 import）；REQUIRED_BEFORE_A5_CLOSE 未全部满足（runtime 接线 / automatic fallback 执行 = 已交付（2026-09-02 AAF-v0.5-A5-FREE-FALLBACK-RUNTIME-001：Hermes executor live path bounded FREE/LOCAL_FREE fallback + authoritative live audit fallback_runtime.json + 30 聚焦测试 + fresh-runner 5/5 + 2026-09-03 FIX-001（ALLOWED_AUTHORIZED_PAID 不进入 FREE fallback invocation + authoritative audit closure 失败不接受 fallback 输出；9 项新聚焦 + fresh-runner FIX-001 4/4；FIX-002（2026-09-03）收口 Codex 唯一 blocker：post-invocation audit closure 异常边界 exception-safe fail-closed：_emit catch 从 (ValueError, TypeError, OSError) 拓宽为 Exception + 兜底边界（RuntimeError/UnicodeError 等未预期 audit 异常统一收口、不裸逃逸；attempted=true / used=false / 输出不被接受 / audit_closure_error surface / 无第三模型 / env 还原；6 项新聚焦 + fresh-runner FIX-002 4/4））；free→paid / Cost Gate UX 单元未开始）；A6（health/quarantine/requalification）与 A4+（HIGH/CRITICAL/Codex routing）边界保持 outside；report = docs/internal/AAF-v0.5-A5-FALLBACK-CONTRACT-001-REPORT.md。**2026-09-02 AAF-v0.5-A5-FREE-FALLBACK-RUNTIME-001（A5 第二条实现单元 = bounded FREE/LOCAL_FREE automatic fallback runtime 接入 Hermes executor live path → A5 保持 STARTED / NOT CLOSED）**：`ai_agent_framework/fallback_runtime.py`（decision = `fallback_contract.decide_fallback` 唯一权威；Requirement-3 cost gate 复用 A3 ACTIVE_ROUTING_COST_CLASSES；candidate admission 复用 A0 Paid Guard；candidate env 覆盖复用 A3 env 契约/restore，无 base_url 时显式清除 stale 覆盖）——runner Hermes stage 原始 invocation 真实失败后至多一次 FREE/LOCAL_FREE fallback attempt（evidence-based 最小失败分类器 classify_failure；exactly-one / no-chain / no-candidate fail closed 保留原始失败 / no silent paid；attempted=true 仅当第二不同模型 invocation 实际发起、used=true 仅当 fallback 输出成为被接受 stage 结果）；authoritative live audit artifact `fallback_runtime.json`（decision_kind=fallback_runtime_audit + validate_fallback_runtime_record fail-closed，与 contract decision record 的 foundation 语义以 decision_kind+authority 明确区分；record 含 original/final actual model-provider、failure class+trigger、candidate、attempted/used、paid_escalation_required、authorization_outcome、no-silent-fallback evidence）；A3 初始 routing / A4 / A0 / fallback_contract.py 零修改；30 项新增聚焦测试（tests/test_a5_fallback_runtime.py：module + 真实 runner 集成全矩阵）+ 全量 non-GUI 4-file-deselect **2026 passed / 1 skipped / 38 deselected**（HEAD 9216426 基线 1996 + 30 精确零回归）+ fresh-runner N+1 5/5（新进程真实 runner + fake CLIs）；REQUIRED_BEFORE_A5_CLOSE 仍未全部满足（free→paid escalation / Cost Gate UX = 已交付 paid escalation Cost Gate authorization-evaluation foundation（2026-09-03 AAF-v0.5-A5-PAID-ESCALATION-GATE-001——显式 A0 授权判断 + authoritative paid_escalation_gate.json 审计 + 零 paid invocation；paid INVOCATION = A5-004 已交付（2026-09-05 AAF-v0.5-A5-PAID-FALLBACK-RUNTIME-001——恰一次 authorized paid fallback invocation + paid_fallback_runtime.json 权威 audit；report = docs/internal/AAF-v0.5-A5-PAID-FALLBACK-RUNTIME-001-REPORT.md）；FIX-001（2026-09-03，parent 32c4bbe）收口 Codex 唯一 blocker：malformed/contradictory A0 authorization fail-closed 后仍生成 validator-valid、可持久化的自洽权威 paid_escalation_gate.json（source_guard_record raw/source 证据分层；14 项新聚焦 + canonical 2115 passed 零回归 + fresh-runner FIX-001 5/5））；REQUIRED_BEFORE_A5_CLOSE 9 项全部满足 → **A5 = CLOSED / COMPLETE / SYNCED**（2026-09-05 AAF-v0.5-A5-CLOSE-001 依 AAF-v0.5-A5-CLOSURE-AUDIT-001（判定 A5_READY_TO_CLOSE）正式关闭（closure record 已接受——2026-09-05 AAF-v0.5-A5-CLOSE-001（786729e）+ FIX-001（cd2e056）经 final WorkBuddy 独立 closure validation（AAF-v0.5-A5-CLOSE-WORKBUDDY-VALIDATION-002 = SUCCESS / PASS_WITH_WARNING / blocking_rework = false）+ final Codex closure（AAF-v0.5-A5-CLOSE-CODEX-FINAL-001 = SUCCESS / APPROVE）accepted、由 AAF-v0.5-A5-CLOSE-SYNC-001 同步 origin/main（SYNCED / cd2e056 / 0/0）——2026-09-05 AAF-v0.5-A5-CLOSURE-STATE-FINALIZE-001 收口，见 PROJECT_STATE「A5 CLOSED」块；FIX-001 时点 route = PENDING / external 429 / precheck REQUEST_CHANGE = 历史时点记录）；closure boundary / 9 项 = PROJECT_STATE「A5 Scope Formalization」块、不重写；上文各单元 NOT CLOSED / paid INVOCATION 未实现叙述 = 实现单元任务时点历史记录）；FREE 单元 report = docs/internal/AAF-v0.5-A5-FREE-FALLBACK-RUNTIME-001-REPORT.md |
| Remaining Gap | A4 正式 scope = **无 remaining gap（LOW + MEDIUM WorkBuddy economic active routing = IMPLEMENTED + CLOSED / COMPLETE——2026-09-02 AAF-v0.5-A4-CLOSE-001 正式关闭，closure 已执行）**；A4+ future scope NOT IMPLEMENTED：HIGH / CRITICAL WorkBuddy routing（HIGH 前置 = T2-or-better WorkBuddy 候选 prerequisite qualification evidence）、multi-agent / Codex routing（前置 = 独立 scope/design 定义）；其余 broader model routing（MEDIUM/HIGH Hermes 自动模型选择与 cheapest-model selection）仍属 A4-A6 未实现；A5 = fallback / escalation / Cost Gate UX（free→paid switching / user confirmation dialog / automatic fallback 等示例全在其内）——completion boundary 已正式化（PROJECT_STATE「A5 Scope Formalization」块 = bounded auditable fallback / escalation / Cost Gate foundation，2026-09-02 AAF-v0.5-A5-SCOPE-FORMALIZATION-001）→ **A5 = STARTED**（2026-09-02 AAF-v0.5-A5-FALLBACK-CONTRACT-001 第一条实现单元 = fallback decision + audit contract foundation 已交付；NOT CLOSED / COMPLETE——正式关闭前置 = REQUIRED_BEFORE_A5_CLOSE 9 项 closure requirements 全部实现并验证，runtime 接线 / 自动 fallback 执行 = 已交付（AAF-v0.5-A5-FREE-FALLBACK-RUNTIME-001；2026-09-03 FIX-001 收口两个 Codex runtime-contract blockers：ALLOWED_AUTHORIZED_PAID 不进入 FREE fallback invocation + authoritative audit closure 失败不接受 fallback 输出）＋FIX-002（2026-09-03）收口 Codex 唯一 blocker：post-invocation audit closure 异常边界 exception-safe fail-closed：_emit catch 从 (ValueError, TypeError, OSError) 拓宽为 Exception + 兜底边界（RuntimeError/UnicodeError 等未预期 audit 异常统一收口、不裸逃逸；6 项新聚焦 + fresh-runner FIX-002 4/4）；free→paid / Cost Gate UX = paid escalation Cost Gate authorization-evaluation foundation 已交付（2026-09-03 AAF-v0.5-A5-PAID-ESCALATION-GATE-001——显式 A0 授权判断 + paid_escalation_gate.json 审计 + 零 paid invocation；paid INVOCATION 未实现 = 后续 A5 scope；FIX-001（2026-09-03，parent 32c4bbe）收口 Codex 唯一 blocker：malformed/contradictory A0 authorization fail-closed 后仍生成 validator-valid、可持久化的自洽权威 paid_escalation_gate.json（source_guard_record raw/source 证据分层；14 项新聚焦 + canonical 2115 passed 零回归 + fresh-runner FIX-001 5/5））——paid escalation 单元未开始（A5-003 时点记录；A5-004 已交付，paid INVOCATION 已实现））；**A5 = CLOSED / COMPLETE / SYNCED**（2026-09-05 AAF-v0.5-A5-CLOSE-001 依 AAF-v0.5-A5-CLOSURE-AUDIT-001（判定 A5_READY_TO_CLOSE）正式关闭（closure record 已接受——2026-09-05 AAF-v0.5-A5-CLOSE-001（786729e）+ FIX-001（cd2e056）经 final WorkBuddy 独立 closure validation（AAF-v0.5-A5-CLOSE-WORKBUDDY-VALIDATION-002 = SUCCESS / PASS_WITH_WARNING / blocking_rework = false）+ final Codex closure（AAF-v0.5-A5-CLOSE-CODEX-FINAL-001 = SUCCESS / APPROVE）accepted、由 AAF-v0.5-A5-CLOSE-SYNC-001 同步 origin/main（SYNCED / cd2e056 / 0/0）——2026-09-05 AAF-v0.5-A5-CLOSURE-STATE-FINALIZE-001 收口，见 PROJECT_STATE「A5 CLOSED」块；FIX-001 时点 route = PENDING / external 429 / precheck REQUEST_CHANGE = 历史时点记录）——REQUIRED_BEFORE_A5_CLOSE 9 项全部满足；completion boundary / 9 项 = PROJECT_STATE「A5 Scope Formalization」块、不重写；上文 A5 = STARTED / NOT CLOSED 叙述 = 实现单元任务时点历史记录）；A6 = observation-calibration-runtime requalification 与 health/quarantine（NOT STARTED；A5 closure 之外 future scope） |
...
| Do Not Forget | A3 LOW slice（LOW-risk Hermes FREE active routing）已实现并关闭；A4 LOW + MEDIUM WorkBuddy economic active routing = IMPLEMENTED + CLOSED / COMPLETE（A4 = CLOSED——2026-09-02 AAF-v0.5-A4-CLOSE-001 正式关闭）；但 **broader Model Routing 未实现**（HIGH / CRITICAL / multi-agent = A4+ future scope；其余 MEDIUM/HIGH 等属 A4-A6）；Free→Paid 切换等决策层必须用户批准（A5 Cost Gate UX——A5 completion boundary 已正式化：bounded auditable fallback / escalation / Cost Gate foundation，A5 = STARTED（2026-09-02 AAF-v0.5-A5-FALLBACK-CONTRACT-001 第一条 foundation 单元 = fallback decision + audit contract 已交付 + 2026-09-02 AAF-v0.5-A5-FREE-FALLBACK-RUNTIME-001 runtime 单元（bounded FREE/LOCAL_FREE automatic fallback 已接入 Hermes executor live path；A5 保持 STARTED / NOT CLOSED——free→paid / Cost Gate UX 未开始）；FIX-001（2026-09-03）收口两个 Codex runtime-contract blockers：ALLOWED_AUTHORIZED_PAID 不进入 FREE fallback invocation（仅 ALLOWED_FREE 可执行；A0 Paid Guard 零修改）+ authoritative audit closure 失败时不接受 fallback 输出（attempted=true / used=false fail closed；FIX-002（2026-09-03）收口 Codex 唯一 blocker：post-invocation audit closure 异常边界 exception-safe fail-closed：_emit catch 从 (ValueError, TypeError, OSError) 拓宽为 Exception + 兜底边界（RuntimeError/UnicodeError 等未预期 audit 异常统一收口、不裸逃逸；attempted=true / used=false / 输出不被接受 / audit_closure_error surface / 无第三模型 / env 还原；6 项新聚焦 + fresh-runner FIX-002 4/4）；修复细节见 PROJECT_STATE A5 块）；NOT CLOSED / COMPLETE——completion boundary = 2026-09-02 AAF-v0.5-A5-SCOPE-FORMALIZATION-001）；free→paid 必经既有 Paid Guard / Cost Guard，无第二套付费授权系统——paid escalation 授权判断 = 2026-09-03 AAF-v0.5-A5-PAID-ESCALATION-GATE-001 Cost Gate 已交付（AUTHORIZED / BLOCKED / FAIL_CLOSED，authorization-evaluation ONLY 零 paid invocation；paid INVOCATION 未开始 = 后续 A5 任务 scope；FIX-001（2026-09-03，parent 32c4bbe）收口 Codex 唯一 blocker：malformed/contradictory A0 authorization fail-closed 后仍生成 validator-valid、可持久化的自洽权威 paid_escalation_gate.json（source_guard_record raw/source 证据分层；14 项新聚焦 + canonical 2115 passed 零回归 + fresh-runner FIX-001 5/5））；**A5 = CLOSED / COMPLETE / SYNCED**（2026-09-05 AAF-v0.5-A5-CLOSE-001 依 AAF-v0.5-A5-CLOSURE-AUDIT-001（判定 A5_READY_TO_CLOSE）正式关闭（closure record 已接受——2026-09-05 AAF-v0.5-A5-CLOSE-001（786729e）+ FIX-001（cd2e056）经 final WorkBuddy 独立 closure validation（AAF-v0.5-A5-CLOSE-WORKBUDDY-VALIDATION-002 = SUCCESS / PASS_WITH_WARNING / blocking_rework = false）+ final Codex closure（AAF-v0.5-A5-CLOSE-CODEX-FINAL-001 = SUCCESS / APPROVE）accepted、由 AAF-v0.5-A5-CLOSE-SYNC-001 同步 origin/main（SYNCED / cd2e056 / 0/0）——2026-09-05 AAF-v0.5-A5-CLOSURE-STATE-FINALIZE-001 收口，见 PROJECT_STATE「A5 CLOSED」块；FIX-001 时点 route = PENDING / external 429 / precheck REQUEST_CHANGE = 历史时点记录）——A5-004 = AAF-v0.5-A5-PAID-FALLBACK-RUNTIME-001（2026-09-05）已交付（paid INVOCATION 已实现）；REQUIRED_BEFORE_A5_CLOSE 9 项全部满足；completion boundary / 9 项 = PROJECT_STATE「A5 Scope Formalization」块、不重写；上文 A5 = STARTED / NOT CLOSED / paid INVOCATION 未开始叙述 = 各实现单元任务时点的历史记录；free→paid 仍必经既有 Paid Guard / Cost Guard 唯一授权 authority、无第二套付费授权系统）** |
...
| Remaining Gap | A1+（A1 及之后各阶段，含 A2+；本任务显式不做，非 A1 单阶段 scope——2026-08-30 CLOSURE-AUDIT-001-FIX-001 裁决注记）：Hermes candidate Tier registry、Selection Engine / Shadow Routing（= A2）、WorkBuddy RemoteConfig 解析与经济路由、free-to-free fallback（= **A5 formal scope**——2026-09-02 AAF-v0.5-A5-SCOPE-FORMALIZATION-001：仅限已 QUALIFIED 候选、free/local→free/local、capability/qualification 先于 cost，见 PROJECT_STATE「A5 Scope Formalization」块）、capability learning、精确 RMB/token 计价、PAID_LOW/MEDIUM/HIGH、Codex 成本优化、图形化 Cost Gate UX（= **A5 formal scope**——须消费本 A0 Paid Guard / Cost Guard authority，**不建第二套付费授权系统**；当前仅最小 blocked-state 文本） |
...
| Do Not Forget | 本 A0 只做 Hermes 保护层；不得据此宣称完整 Model Routing 已实现（CAP-003 完整 Model Routing 仍 NOT IMPLEMENTED，属 A4-A6 与 A4+ future scope；已实现并关闭的仅 A3 LOW-risk Hermes FREE slice + A4 LOW + MEDIUM WorkBuddy economic slice（A4 = CLOSED / COMPLETE——2026-09-02 AAF-v0.5-A4-CLOSE-001 正式关闭）；A5（fallback / escalation / Cost Gate foundation） completion boundary 已正式化 = STARTED（2026-09-02 AAF-v0.5-A5-FALLBACK-CONTRACT-001 foundation 单元 = fallback decision + audit contract 已交付 + 2026-09-02 AAF-v0.5-A5-FREE-FALLBACK-RUNTIME-001 runtime 单元（bounded FREE/LOCAL_FREE automatic fallback 已接入 Hermes executor live path；A5 保持 STARTED / NOT CLOSED——free→paid / Cost Gate UX 未开始）；FIX-001（2026-09-03）收口两个 Codex runtime-contract blockers：ALLOWED_AUTHORIZED_PAID 不进入 FREE fallback invocation（仅 ALLOWED_FREE 可执行；A0 Paid Guard 零修改）+ authoritative audit closure 失败时不接受 fallback 输出（attempted=true / used=false fail closed；FIX-002（2026-09-03）收口 Codex 唯一 blocker：post-invocation audit closure 异常边界 exception-safe fail-closed：_emit catch 从 (ValueError, TypeError, OSError) 拓宽为 Exception + 兜底边界（RuntimeError/UnicodeError 等未预期 audit 异常统一收口、不裸逃逸；attempted=true / used=false / 输出不被接受 / audit_closure_error surface / 无第三模型 / env 还原；6 项新聚焦 + fresh-runner FIX-002 4/4）；修复细节见 PROJECT_STATE A5 块），**A5 = CLOSED / COMPLETE / SYNCED**（2026-09-05 AAF-v0.5-A5-CLOSE-001 依 AAF-v0.5-A5-CLOSURE-AUDIT-001（判定 A5_READY_TO_CLOSE）正式关闭（closure record 已接受——2026-09-05 AAF-v0.5-A5-CLOSE-001（786729e）+ FIX-001（cd2e056）经 final WorkBuddy 独立 closure validation（AAF-v0.5-A5-CLOSE-WORKBUDDY-VALIDATION-002 = SUCCESS / PASS_WITH_WARNING / blocking_rework = false）+ final Codex closure（AAF-v0.5-A5-CLOSE-CODEX-FINAL-001 = SUCCESS / APPROVE）accepted、由 AAF-v0.5-A5-CLOSE-SYNC-001 同步 origin/main（SYNCED / cd2e056 / 0/0）——2026-09-05 AAF-v0.5-A5-CLOSURE-STATE-FINALIZE-001 收口，见 PROJECT_STATE「A5 CLOSED」块；FIX-001 时点 route = PENDING / external 429 / precheck REQUEST_CHANGE = 历史时点记录）——REQUIRED_BEFORE_A5_CLOSE 9 项全部满足；A5-004 = AAF-v0.5-A5-PAID-FALLBACK-RUNTIME-001（2026-09-05）已交付；上文 NOT CLOSED / COMPLETE 叙述 = 实现单元任务时点历史记录）；completion boundary = 2026-09-02 AAF-v0.5-A5-SCOPE-FORMALIZATION-001——free→paid 仍必须走本 A0 guard / Cost Guard，A5 不建第二套付费授权（A5-004 恰一次 invocation 语义保持）））；授权 env 是 per-run 一次性，不是永久/全局 bypass |
...
| MVP-001 | v0.5 Personal MVP Freeze（个人轻量 Agent 协作调度 MVP：产品定义 / 能力边界 / freeze criteria / post-freeze opt-in scope policy；权威 = PROJECT_STATE v0.5「MVP FROZEN」块） | FROZEN / CLOSED (v0.5, 2026-09-05) | P1 |
| PH-001 | v0.5 PH-1 Portability Hardening：Planner bootstrap + replaceable role contracts + 收口（入口 = START_HERE_FOR_NEW_PLANNER.md；权威 = docs/internal/AAF_ROLE_CONTRACTS.md；record = docs/internal/AAF-v0.5-PH1-PLANNER-BOOTSTRAP-ROLE-CONTRACTS-001-REPORT.md；closure record = docs/internal/AAF-v0.5-PH1-PORTABILITY-CLOSE-001-REPORT.md；Gap Audit = PH1_RUNTIME_GAPS_NOT_BLOCKING） | CLOSED / COMPLETE (PH-1, docs-only, 2026-09-05) | P1 |
| CAP-001 | Model Observability | IMPLEMENTED | P1 |
| CAP-002 | Model Discovery（以真实 Agent 接口为限） | IMPLEMENTED | P1 |
| CAP-003 | Future Model Routing（政策登记；A3 LOW-risk Hermes FREE active routing = IMPLEMENTED + CLOSED；A4 LOW + MEDIUM WorkBuddy economic active routing = IMPLEMENTED + CLOSED / COMPLETE（A4 = CLOSED——2026-09-02 AAF-v0.5-A4-CLOSE-001）；HIGH/CRITICAL/multi-agent = A4+ future scope NOT IMPLEMENTED；其余 broader model routing 属 A4-A6；A5（fallback / escalation / Cost Gate foundation）= STARTED（2026-09-02 AAF-v0.5-A5-FALLBACK-CONTRACT-001 foundation 单元 = fallback decision + audit contract 已交付 + 2026-09-02 AAF-v0.5-A5-FREE-FALLBACK-RUNTIME-001 runtime 单元（bounded FREE/LOCAL_FREE automatic fallback 已接入 Hermes executor live path；A5 保持 STARTED / NOT CLOSED——free→paid / Cost Gate UX 未开始）；FIX-001（2026-09-03）收口两个 Codex runtime-contract blockers：ALLOWED_AUTHORIZED_PAID 不进入 FREE fallback invocation（仅 ALLOWED_FREE 可执行；A0 Paid Guard 零修改）+ authoritative audit closure 失败时不接受 fallback 输出（attempted=true / used=false fail closed；FIX-002（2026-09-03）收口 Codex 唯一 blocker：post-invocation audit closure 异常边界 exception-safe fail-closed：_emit catch 从 (ValueError, TypeError, OSError) 拓宽为 Exception + 兜底边界（RuntimeError/UnicodeError 等未预期 audit 异常统一收口、不裸逃逸；attempted=true / used=false / 输出不被接受 / audit_closure_error surface / 无第三模型 / env 还原；6 项新聚焦 + fresh-runner FIX-002 4/4）；修复细节见 PROJECT_STATE A5 块）；**A5 = CLOSED / COMPLETE / SYNCED**（2026-09-05 AAF-v0.5-A5-CLOSE-001 依 CLOSURE-AUDIT-001（判定 A5_READY_TO_CLOSE）正式关闭（closure record 已接受——2026-09-05 AAF-v0.5-A5-CLOSE-001（786729e）+ FIX-001（cd2e056）经 final WorkBuddy 独立 closure validation（AAF-v0.5-A5-CLOSE-WORKBUDDY-VALIDATION-002 = SUCCESS / PASS_WITH_WARNING / blocking_rework = false）+ final Codex closure（AAF-v0.5-A5-CLOSE-CODEX-FINAL-001 = SUCCESS / APPROVE）accepted、由 AAF-v0.5-A5-CLOSE-SYNC-001 同步 origin/main（SYNCED / cd2e056 / 0/0）——2026-09-05 AAF-v0.5-A5-CLOSURE-STATE-FINALIZE-001 收口，见 PROJECT_STATE「A5 CLOSED」块；FIX-001 时点 route = PENDING / external 429 / precheck REQUEST_CHANGE = 历史时点记录）——REQUIRED_BEFORE_A5_CLOSE 9 项全部满足；A5-004 = AAF-v0.5-A5-PAID-FALLBACK-RUNTIME-001（2026-09-05）已交付；completion boundary / 9 项 = PROJECT_STATE「A5 Scope Formalization」块、不重写）**；**2026-09-03 AAF-v0.5-A5-PAID-ESCALATION-GATE-001（paid escalation Cost Gate runtime foundation，A5 保持 STARTED / NOT CLOSED）**：fallback-eligible 失败 + 无合格 FREE/LOCAL_FREE fallback + 合格 paid candidate → 既有 A0 Paid Guard 显式授权判断（gate AUTHORIZED / BLOCKED / FAIL_CLOSED；authorization-evaluation ONLY 零 paid invocation；audit = paid_escalation_gate.json） | IMPLEMENTED（A3 LOW slice + A4 LOW/MEDIUM WorkBuddy slice，均已关闭；HIGH/CRITICAL/multi-agent = A4+ 与 MEDIUM-HIGH Hermes/fallback/health 等属 A4-A6 未实现；A5 = CLOSED / COMPLETE / SYNCED（2026-09-05 AAF-v0.5-A5-CLOSE-001 依 CLOSURE-AUDIT-001（A5_READY_TO_CLOSE）正式关闭（closure record 已接受——2026-09-05 AAF-v0.5-A5-CLOSE-001（786729e）+ FIX-001（cd2e056）经 final WorkBuddy 独立 closure validation（AAF-v0.5-A5-CLOSE-WORKBUDDY-VALIDATION-002 = SUCCESS / PASS_WITH_WARNING / blocking_rework = false）+ final Codex closure（AAF-v0.5-A5-CLOSE-CODEX-FINAL-001 = SUCCESS / APPROVE）accepted、由 AAF-v0.5-A5-CLOSE-SYNC-001 同步 origin/main（SYNCED / cd2e056 / 0/0）——2026-09-05 AAF-v0.5-A5-CLOSURE-STATE-FINALIZE-001 收口，见 PROJECT_STATE「A5 CLOSED」块；FIX-001 时点 route = PENDING / external 429 / precheck REQUEST_CHANGE = 历史时点记录）；boundary 已正式化、A5-001..004 四实现单元全部交付（含 A5-004 = PAID-FALLBACK-RUNTIME-001））；A6 未实现） | P2 |
