...
| Status | **IMPLEMENTED (A0)**（v0.5 A0 交付；2026-08-29 FIX-002 修复 Codex 三个 blocking fail-open 发现：①本地端点改 hostname/IP 严格判定（substring 移除）；②移除 `AAF_COST_FREE_MODELS` 权威 FREE 路径（A0 无远程 FREE 权威）；③授权改为**准入即消费**的真一次性语义（replay 拒绝、fail closed）。正式 COMPLETE 判定留待 WorkBuddy 复验 + Codex re-review + Planner 确认） |
| Priority | P1 |
