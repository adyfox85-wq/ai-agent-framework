"""AAF-v0.5-A1-CLOSURE-PROTOCOL-CORRECTION-001-FIX-001 fresh-runner validation
driver（Run N+1）。

Requirement 17/18：本任务修改 Framework result/provenance authority——Run N
无法自证修正后的 post-stage artifact 语义，必须用**一次** fresh-runner N+1
独立验证（不叠加冗余 N+1 循环）。每个场景用全新 python 进程运行真实 runner
...
N+1 必须独立证明（Requirement 17）：
1. 一个创建真实 commit 的任务，其归一化 hermes_result.json：
   - commit == 实际 post-stage HEAD
   - commit_changed == true
   - changed_files 含实际已提交的 task path（worktree 干净后不再塌缩为 []）
   - raw agent 自报 changed_files=[] 不影响归一化（Framework Git 观察权威）
   - PRE_ALLOWED_UNTRACKED 常驻项不进入 changed_files
...
用法：python tests/fresh_runner_changed_files_provenance.py
退出码 = 失败场景数（0 = 通过）。证据写入
.aaf/AAF-v0.5-A1-CLOSURE-PROTOCOL-CORRECTION-001-FIX-001/fresh-runner-validation/
...
DEFAULT_EVIDENCE_ROOT = (
    ROOT / ".aaf" / "AAF-v0.5-A1-CLOSURE-PROTOCOL-CORRECTION-001-FIX-001"
...
TASK_ID = "AAF-CHANGED-FILES-N1"
TASK_TEXT = f"""# Task ID
{TASK_ID}

# Task Name
fresh-runner changed-files provenance N+1 validation

# Objective
验证归一化 changed_files 在 stage 创建真实 commit 后不再塌缩为 []

# Acceptance
1. framework 任务执行成功
2. 归一化 hermes_result.json 的 commit / commit_changed / changed_files 与 Git 事实一致
3. REPORT / lifecycle 生成健康

Route: hermes
"""

# fake hermes.bat（ASCII only；cmd 元字符不在注释/内容中出现）：
# - `hermes --version` / `--help` / `config get model` -> probe 应答（cost_guard
#   经 patched discovery PATH 会走到本 fake；model_observation 走真实 PATH，
#   非阻塞）
# - `hermes chat ...` -> 在 %CD%（= workspace）修改 tracked 文件并**真实 commit**，
#   写 marker（真实 child 证据），然后输出 raw structured result
#   （changed_files 自报 [] —— 与父任务缺陷场景一致：归一化必须由 Framework
#   Git 观察覆盖，不依赖 agent 自报）。
...
rem --- executor path: make a real commit in the workspace (%CD%) ---
echo stage output v2 > docs\\n1_stage.md
git add docs/n1_stage.md
git commit -q -m "N1 provenance fixture commit"
if defined FAKE_HERMES_MARKER (
  echo SPAWNED > "%FAKE_HERMES_MARKER%"
)
echo FAKE HERMES EXECUTOR: changed-files N+1 validation stub (no real inference)
echo.
...
def _run_scenario(root: Path) -> tuple[bool, list[str]]:
    ...
    _check("changed_files.has_committed_path", "M docs/n1_stage.md" in changed,
           f"changed_files={changed}（worktree 干净后不得塌缩为 []）")
    ...
    _check("report.remote_sync.tree_clean",
           "Tracked Working Tree: CLEAN" in report,
           "REPORT tracked tree 未如实上报 CLEAN")
    # 无 upstream 的 scratch 仓库：commit_sync=UNKNOWN / task_remote_sync=UNSYNCED
    # 是如实上报（不虚构 SYNCED）
...
def main() -> int:
    ...
    print("=" * 72)
    print("AAF-v0.5-A1-CLOSURE-PROTOCOL-CORRECTION-001-FIX-001 — fresh-runner "
