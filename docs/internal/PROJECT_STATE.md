...
- hotkey listener 偶发失活已实现自动自恢复（RW-012 = SOLVED for v0.4：Bridge 唯一 owner 有界 backoff，失败经 Tray/状态窗口可见；FIX-001 后 registration/unregistration 归 listener 线程（thread-owned unregister）+ 显式 stop 契约（有界 join）+ stop-before-replace，one-listener invariant 有真实线程级证据）；
