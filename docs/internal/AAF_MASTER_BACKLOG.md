...
| CAP-003 | Future Model Routing（政策登记；A3 LOW-risk Hermes FREE active routing = IMPLEMENTED + CLOSED；A4 LOW + MEDIUM WorkBuddy economic active routing = IMPLEMENTED + CLOSED / COMPLETE（A4 = CLOSED——2026-09-02 AAF-v0.5-A4-CLOSE-001）；HIGH/CRITICAL/multi-agent = A4+ future scope NOT IMPLEMENTED；其余 broader model routing 属 A4-A6；A5（fallback / escalation / Cost Gate foundation）= STARTED（2026-09-02 AAF-v0.5-A5-FALLBACK-CONTRACT-001 foundation 单元 = fallback decision + audit contract 已交付 + 2026-09-02 AAF-v0.5-A5-FREE-FALLBACK-RUNTIME-001 runtime 单元（bounded FREE/LOCAL_FREE automatic fallback 已接入 Hermes executor live path；A5 保持 STARTED / NOT CLOSED——free→paid / Cost Gate UX 未开始）；FIX-001（2026-09-03）收口两个 Codex runtime-contract blockers：ALLOWED_AUTHORIZED_PAID 不进入 FREE fallback invocation（仅 ALLOWED_FREE 可执行；A0 Paid Guard 零修改）+ authoritative audit closure 失败时不接受 fallback 输出（attempted=true / used=false fail closed；FIX-002（2026-09-03）收口 Codex 唯一 blocker：post-invocation audit closure 异常边界 exception-safe fail-closed：_emit catch 从 (ValueError, TypeError, OSError) 拓宽为 Exception + 兜底边界（RuntimeError/UnicodeError 等未预期 audit 异常统一收口、不裸逃逸；attempted=true / used=false / 输出不被接受 / audit_closure_error surface / 无第三模型 / env 还原；6 项新聚焦 + fresh-runner FIX-002 4/4）；修复细节见 PROJECT_STATE A5 块）；NOT CLOSED / COMPLETE）；**2026-09-03 AAF-v0.5-A5-PAID-ESCALATION-GATE-001（paid escalation Cost Gate runtime foundation，A5 保持 STARTED / NOT CLOSED）**：fallback-eligible 失败 + 无合格 FREE/LOCAL_FREE fallback + 合格 paid candidate → 既有 A0 Paid Guard 显式授权判断（gate AUTHORIZED / BLOCKED / FAIL_CLOSED；authorization-evaluation ONLY 零 paid invocation；audit = paid_escalation_gate.json） | IMPLEMENTED（A3 LOW slice + A4 LOW/MEDIUM WorkBuddy slice，均已关闭；HIGH/CRITICAL/multi-agent = A4+ 与 MEDIUM-HIGH Hermes/fallback/health 等属 A4-A6 未实现；A5 = STARTED（boundary 已正式化、foundation 单元 001 + runtime 单元（FREE-FALLBACK-RUNTIME-001 + PAID-ESCALATION-GATE-001）已交付、未 CLOSED）；A6 未实现） | P2 |
