...
Next mainline: v0.5 -> A2 Shadow Routing（未启动、未实现；A3 Hermes Free Auto Routing 未启动；
  Hermes free-model 可用性/稳定性为后续路由（A2+ 影子/实况观测）的 runtime qualification
  concern，不假定 FREE = healthy；详见本 FIX 报告与 AAF-v0.5-A1-CLOSURE-AUDIT-001-FIX-001 REPORT）
