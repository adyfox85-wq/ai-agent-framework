...
Status: A0 = CLOSED / COMPLETE（2026-08-30，AAF-v0.5-A0-PAID-GUARD-001-CLOSE-001 正式关闭：
  ...
  工作树 tracked CLEAN（untracked 仅 PRE_ALLOWED_UNTRACKED 常驻项）；A1 = STARTED）
...
Next (A1 remaining slices): Selection Engine / Shadow Routing / Hermes candidate tier
  赋值 / 运行时 qualification 观测——A1 未 full closed（foundation 已交付，剩余计划
  slices 未完成）；A2（Shadow Routing）/ A3（Hermes Free Auto Routing）未启动；Hermes
  free-model 可用性/稳定性为后续路由的 runtime qualification concern，不假定 FREE = healthy
