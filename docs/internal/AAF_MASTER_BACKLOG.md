...
| Status | **IMPLEMENTED（以真实 Agent 接口为限）** |
| Priority | P1 |
| Evidence / Origin | 真实 CLI 只读 probe（TASK-010 Requirement 21）：Hermes v0.20.5（`config get model` / `config get auxiliary` / `status`）；CodeBuddy 2.137.1（`config get model` / `--help`）；codex-cli 0.150.0-alpha.12.2（`exec --help` / `~/.codex/config.toml`） |
