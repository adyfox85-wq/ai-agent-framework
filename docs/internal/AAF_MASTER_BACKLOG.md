...
| Current Implementation | A3 actual routing **STARTED（首片已交付，未 COMPLETE）**——2026-09-01 AAF-v0.5-A3-HERMES-FREE-ROUTING-001：`ai_agent_framework/active_routing.py`（authoritative routing decision，**复用 A2 selector / A1 registry / risk 契约，不创建第二套路由判断**）+ runner Hermes stage 集成（Paid Guard 求值前决策 + AAF_HERMES_MODEL/PROVIDER/BASE_URL 覆盖，observation 后还原）+ cost_guard `AAF_HERMES_BASE_URL` 端点事实支持 + model_registry base_url 字段；仅当 Hermes executor task 显式 Risk: LOW + selector eligible + selected 候选 FREE/LOCAL_FREE + QUALIFIED → routing_applied=true → 真实 invocation 指向 qwen3:4b@custom（-m/--provider 透传，guard ALLOWED_FREE 零授权——Paid Guard 未被绕过）；Risk 缺失 / MEDIUM/HIGH/CRITICAL / 无候选 / 非 FREE → 保持 configured deepseek-v4-flash@deepseek；无 silent fallback（fallback_attempted 恒 false）；审计 = active_routing.json（authoritative=true）与 shadow_observation.json（authoritative=false）并存区分；31 项聚焦测试 + 全量 non-GUI 零回归（stash 反证 1595/1 基线 + 31 新增 = 1626 全过）+ fresh-runner N+1 通过（N1 LOW 真实路由到 qwen3 / N2 HIGH control 保持 deepseek）；MEDIUM/HIGH 自动模型选择 / automatic fallback / health polling / quarantine / automatic qualification promotion / WorkBuddy-Codex routing / A4-A6 **均未实现**；A3 正式 COMPLETE 判定留待 WorkBuddy 独立验证 + Codex APPROVE + Planner。注（2026-08-31，AAF-v0.5-A2-SHADOW-ROUTING-001）：A2 Shadow Routing 已 STARTED——`ai_agent_framework/shadow_routing.py` deterministic **shadow-selection engine**（假设性 / 非权威 / 零执行影响，live 路径零 import）；A2-002 接入 Hermes shadow observation（observation-only bypass）；A2-003 建立 explicit task-risk provenance（TASK 可选 `Risk` 字段 → immutable snapshot → shadow observation，Planner 显式结构化声明，缺失 = RISK_UNAVAILABLE，非法值严格拒绝）；A2-004 为 deepseek-v4-flash@deepseek 填入证据支持的 registry qualification（capability_tier=T2 + QUALIFIED，仅来自已接受的 003-FIX-001 HIGH 执行/审查证据，accepted evidence snapshot、不推断 T1/T0、cost_class 仍 UNKNOWN）——HIGH Hermes shadow 现产生真实 hypothetical candidate；本 CAP-003 指**实际** Model Routing（切换 / fallback / 升级），A2 阶段仍 NOT IMPLEMENTED |
...
| CAP-003 | Future Model Routing（政策登记；A3 首片 = LOW-risk FREE active routing 已交付，A3 = STARTED 未 COMPLETE） | STARTED（首片交付；MEDIUM/HIGH/fallback/health 等未实现） | P2 |
