...
| Related | RW-007（Agent executable discovery reliability——MISSING_COMMAND 仍永久失败不重试）；CAP-003（Future Model Routing——A3 LOW-risk Hermes FREE active routing = IMPLEMENTED + CLOSED；A4 LOW + MEDIUM WorkBuddy economic active routing = IMPLEMENTED（A4 = READY_TO_CLOSE）；HIGH / CRITICAL WorkBuddy 与 multi-agent / Codex routing = A4+ future scope NOT IMPLEMENTED；其余 broader model routing 属 A4-A6） |
...
## CAP-003 — Future Model Routing（能力登记：A3 LOW-risk Hermes FREE active routing = IMPLEMENTED + CLOSED；A4 LOW + MEDIUM WorkBuddy economic active routing = IMPLEMENTED（A4 = READY_TO_CLOSE，正式 completion boundary 见 PROJECT_STATE.md v0.5「A4 Scope Formalization」块）；HIGH / CRITICAL WorkBuddy 与 multi-agent / Codex routing = A4+ future scope NOT IMPLEMENTED；其余 broader model routing 属 A4-A6 NOT IMPLEMENTED）
...
| Do Not Forget | A3 LOW slice（LOW-risk Hermes FREE active routing）已实现并关闭；A4 LOW + MEDIUM WorkBuddy economic active routing = IMPLEMENTED（A4 = READY_TO_CLOSE，未正式关闭——正式 closure 为独立 Planner-approved task）；但 **broader Model Routing 未实现**（HIGH / CRITICAL / multi-agent = A4+ future scope；其余 MEDIUM/HIGH 等属 A4-A6）；Free→Paid 切换等决策层必须用户批准（A5 Cost Gate UX） |
...
| Do Not Forget | 本 A0 只做 Hermes 保护层；不得据此宣称完整 Model Routing 已实现（CAP-003 完整 Model Routing 仍 NOT IMPLEMENTED，属 A4-A6 与 A4+ future scope；已实现并关闭的仅 A3 LOW-risk Hermes FREE slice + A4 LOW + MEDIUM WorkBuddy economic slice（A4 = READY_TO_CLOSE，未正式关闭））；授权 env 是 per-run 一次性，不是永久/全局 bypass |
...
| CAP-003 | Future Model Routing（政策登记；A3 LOW-risk Hermes FREE active routing = IMPLEMENTED + CLOSED；A4 LOW + MEDIUM WorkBuddy economic active routing = IMPLEMENTED（A4 = READY_TO_CLOSE）；HIGH/CRITICAL/multi-agent = A4+ future scope NOT IMPLEMENTED；其余 broader model routing 属 A4-A6） | IMPLEMENTED（A3 LOW slice + A4 LOW/MEDIUM WorkBuddy slice；HIGH/CRITICAL/multi-agent = A4+ 与 MEDIUM-HIGH Hermes/fallback/health 等属 A4-A6 未实现） | P2 |
